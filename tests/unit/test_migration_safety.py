...
def test_create_backup(migration_safety, temp_db):
    ...
    backup = migration_safety.create_backup(
        migration_revision="abc123def456", description="Test backup"
    )
...
def test_list_backups(migration_safety):
    ...
    backup2 = migration_safety.create_backup(migration_revision="xyz789", description="Backup 2")
...
def test_run_safety_checks_all_pass(migration_safety, temp_db):
    ...
    with patch("shutil.disk_usage") as mock_disk:
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.return_value = Mock(returncode=0, stdout="alembic 1.13.0")
...
def test_run_safety_checks_alembic_not_installed(migration_safety, temp_db):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        alembic_check = next((c for c in checks if c.check_name == "alembic_installed"), None)
...
def test_run_safety_checks_git_status(migration_safety, temp_db):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        git_check = next((c for c in checks if c.check_name == "git_status"), None)
        if git_check:
...
def test_validate_migration_script_not_found(migration_safety):
    ...
    is_valid, issues = migration_safety.validate_migration_script(Path("/nonexistent/script.py"))
    assert is_valid is False
...
def test_create_rollback_plan(migration_safety):
    ...
    plan = migration_safety.create_rollback_plan(from_revision="abc123", to_revision="def456")
...
def test_create_rollback_plan_to_base(migration_safety):
    ...
    plan = migration_safety.create_rollback_plan(from_revision="xyz789", to_revision="base")
