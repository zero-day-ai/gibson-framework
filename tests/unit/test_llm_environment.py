...
class TestEnvironmentManager:
    ...
    @pytest.mark.asyncio
    async def test_discover_providers_invalid_configuration(
        self, invalid_openai_environment: Dict[str, str]
    ):
    ...
    @pytest.mark.asyncio
    async def test_discover_providers_multi_provider(
        self, multi_provider_environment: Dict[str, str]
    ):
    ...
    async def test_generate_environment_file(self):
        ...
        env_file = await manager.generate_environment_file(
            providers=[LLMProvider.OPENAI, LLMProvider.ANTHROPIC], include_examples=True
        )
    ...
    async def test_generate_environment_file_no_examples(self):
        ...
        manager = EnvironmentManager({})
        env_file = await manager.generate_environment_file(
            providers=[LLMProvider.OPENAI], include_examples=False
        )
...
class TestProviderSpecificValidation:
    ...
    @pytest.mark.parametrize(
        "provider,env_vars,should_be_valid",
        [
            # OpenAI valid
            (
                LLMProvider.OPENAI,
            ...
            (LLMProvider.OPENAI, {"OPENAI_API_KEY": "invalid-key"}, False),
            # Anthropic valid
            (
                LLMProvider.ANTHROPIC,
                {"ANTHROPIC_API_KEY": "sk-ant-1234567890abcdef1234567890abcdef1234567890abcdef"},
            ...
            (
                ...
                {"VERTEX_PROJECT": "my-project-12345", "VERTEX_LOCATION": "us-central1"},
                True,
            ),
            # Replicate valid
            (
        ...
        for provider, patterns in manager.PROVIDER_PATTERNS.items():
            for pattern in patterns:
                if pattern.required:
                    ...
                    assert (
                        len(pattern.description) > 5
                    ), f"Description too short for {provider}.{pattern.name}"
