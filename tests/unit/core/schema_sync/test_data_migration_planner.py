...
class TestDataMigrationPlanner:
    ...
    def test_plan_simple_type_conversion(self):
        ...
        assert (
            "CAST" in transformation.sql_template
            or "convert" in transformation.sql_template.lower()
        )
    ...
    def test_plan_field_removal_with_backup(self):
        ...
        changeset = ChangeSet(
            removed_fields=["legacy_data"], model_hash_before="hash1", model_hash_after="hash2"
        )
        ...
        assert any(t.transformation_type == TransformationType.BACKUP for t in plan.transformations)

    def test_plan_complex_migration(self):
        """Test planning for complex migration with multiple changes."""
        changeset = ChangeSet(
            added_fields={
                "status": FieldInfo(name="status", type="string", nullable=False, default="active")
            },
    ...
    def test_plan_enum_value_migration(self):
        ...
        assert plan is not None
        assert any(
            t.transformation_type == TransformationType.VALUE_MAPPING for t in plan.transformations
        )
    ...
    def test_estimate_risk_level(self):
        ...
        changeset_low = ChangeSet(
            added_fields={"optional": FieldInfo(name="optional", type="string", nullable=True)},
            model_hash_before="hash1",
            model_hash_after="hash2",
        )
        ...
        changeset_high = ChangeSet(
            removed_fields=["important_field"], model_hash_before="hash1", model_hash_after="hash2"
        )
    ...
    def test_data_loss_detection(self):
        ...
        changeset_removal = ChangeSet(
            removed_fields=["data_field"], model_hash_before="hash1", model_hash_after="hash2"
        )
    ...
    def test_migration_phases(self):
        ...
        changeset = ChangeSet(
            added_fields={
                "new_col": FieldInfo(
                    name="new_col", type="string", nullable=False, default="default"
                )
            },
    ...
    def test_sql_template_generation(self):
        ...
        assert any(
            keyword in transformation.sql_template.upper() for keyword in ["CAST", "CONVERT", "::"]
        )
        ...
        changeset = ChangeSet(
            added_fields={
                "test_field": FieldInfo(
                    name="test_field", type="boolean", nullable=False, default=False
                )
            },
            model_hash_before="hash1",
            model_hash_after="hash2",
