...
class TestRateLimiter:
    ...
    async def test_acquire_success(self, rate_limiter):
        ...
        success = await rate_limiter.acquire(provider=LLMProvider.OPENAI, estimated_tokens=100)
    ...
    async def test_acquire_rate_limited(self, rate_limiter):
        ...
        for _ in range(10):
            await rate_limiter.acquire(provider=LLMProvider.OPENAI, estimated_tokens=50)

        # Next request should be rate limited
        success = await rate_limiter.acquire(
            provider=LLMProvider.OPENAI, estimated_tokens=50, timeout=0.1
        )
    ...
    async def test_release(self, rate_limiter):
        ...
        token = await rate_limiter.acquire(provider=LLMProvider.OPENAI, estimated_tokens=100)
        assert token is not None

        await rate_limiter.release(provider=LLMProvider.OPENAI, success=True, actual_tokens=95)
    ...
    async def test_concurrent_limit(self, rate_limiter):
        ...
        token1 = await rate_limiter.acquire(provider=LLMProvider.OPENAI, estimated_tokens=100)
        token2 = await rate_limiter.acquire(provider=LLMProvider.OPENAI, estimated_tokens=100)
        ...
        token3_task = asyncio.create_task(
            rate_limiter.acquire(provider=LLMProvider.OPENAI, estimated_tokens=100, timeout=0.1)
        )
    ...
    async def test_priority_queuing(self, rate_limiter):
        ...
        await rate_limiter.acquire(provider=LLMProvider.OPENAI, estimated_tokens=100)
        await rate_limiter.acquire(provider=LLMProvider.OPENAI, estimated_tokens=100)
        ...
        # High priority should complete first
            ...
            [low_priority, high_priority], return_when=asyncio.FIRST_COMPLETED
        )
    ...
    async def test_module_limits(self, rate_limiter):
        ...
        success = await rate_limiter.acquire(
            provider=LLMProvider.OPENAI, estimated_tokens=100, module_name="test_module"
        )
        assert success is not None
...
class TestBackpressureManager:
    ...
    def backpressure_manager(self):
        ...
        return BackpressureManager(threshold_percentage=80, critical_percentage=95)

    def test_calculate_pressure(self, backpressure_manager):
        """Test pressure calculation."""
        pressure = backpressure_manager.calculate_pressure(current_load=50, max_capacity=100)
        assert pressure == 50.0

        pressure = backpressure_manager.calculate_pressure(current_load=80, max_capacity=100)
    ...
    async def test_apply_backpressure_allow(self, backpressure_manager):
        ...
        result = await backpressure_manager.apply_backpressure(action=BackpressureAction.ALLOW)
    ...
    async def test_apply_backpressure_throttle(self, backpressure_manager):
        ...
        result = await backpressure_manager.apply_backpressure(
            action=BackpressureAction.THROTTLE, delay_ms=100
        )
    ...
    async def test_apply_backpressure_reject(self, backpressure_manager):
        ...
        result = await backpressure_manager.apply_backpressure(action=BackpressureAction.REJECT)
        assert result is False
