...
from gibson.core.targets.manager import TargetManager, TargetManagerError, TargetValidationError
...
class TestTargetManager:
    ...
    async def test_create_target_duplicate_name_raises_error(self, target_manager):
        ...
        # Create first target
        await target_manager.create_target(name="duplicate", base_url="https://api.test.com")

        # Try to create second with same name
        with pytest.raises(TargetAlreadyExistsError):
            await target_manager.create_target(name="duplicate", base_url="https://api.other.com")

    @pytest.mark.asyncio
    async def test_get_target_by_id(self, target_manager):
        """Test getting target by ID."""
        created = await target_manager.create_target(
            name="test-get", base_url="https://api.test.com"
        )
    ...
    async def test_get_target_by_name(self, target_manager):
        ...
        created = await target_manager.create_target(
            name="test-get-name", base_url="https://api.test.com"
        )
    ...
    async def test_update_target(self, target_manager):
        ...
        created = await target_manager.create_target(
            name="test-update", base_url="https://api.test.com"
        )

        updated = await target_manager.update_target(
            created, description="Updated description", status=TargetStatus.ACTIVE
        )
    ...
    async def test_delete_target(self, target_manager, mock_credential_manager):
        ...
        created = await target_manager.create_target(
            name="test-delete", base_url="https://api.test.com"
        )
    ...
    async def test_search_targets(self, target_manager):
        ...
        await target_manager.create_target(
            name="search-test", base_url="https://api.search.com", description="Searchable target"
        )
    ...
    async def test_validate_target(self, target_manager, mock_litellm_adapter):
        ...
        created = await target_manager.create_target(
            name="validate-test", base_url="https://api.test.com"
        )
        ...
        with patch("gibson.core.targets.manager.aiohttp.ClientSession") as mock_session:
            ...
            mock_session.return_value.__aenter__.return_value.head.return_value.__aenter__.return_value = (
                mock_response
            )

            result = await target_manager.validate_target(
                created.id, test_connection=True, validate_credentials=False
            )
    ...
    async def test_set_target_credential(self, target_manager, mock_credential_manager):
        ...
        created = await target_manager.create_target(
            name="cred-test", base_url="https://api.test.com"
        )

        success = await target_manager.set_target_credential(
            created.id, api_key="sk-test123", key_format=ApiKeyFormat.BEARER_TOKEN
        )
    ...
    async def test_remove_target_credential(self, target_manager, mock_credential_manager):
        ...
        created = await target_manager.create_target(
            name="cred-remove-test", base_url="https://api.test.com"
        )
    ...
    async def test_export_targets(self, target_manager, tmp_path):
        ...
        for i in range(2):
            await target_manager.create_target(
                name=f"export-test-{i}", base_url=f"https://api{i}.test.com"
            )
    ...
    async def test_get_statistics(self, target_manager):
        ...
        for i in range(3):
            await target_manager.create_target(
                name=f"stats-test-{i}", base_url=f"https://api{i}.test.com"
            )
