...
class TestPublicRepositoryOperations:
    ...
    async def test_clone_with_specific_branch(self, git_sync):
        ...
        # Clone main branch
        result = await git_sync.clone_repository(git_url, branch="main")
...
class TestAuthenticationEscalation:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(
        not os.getenv("GIBSON_TEST_PRIVATE_REPO"), reason="Private repo URL not provided"
    )
...
class TestPerformanceAndOptimization:
    ...
    async def test_sparse_checkout(self, git_sync):
        ...
        result = await git_sync.clone_repository(git_url, sparse_patterns=["*.py", "*.md"])
