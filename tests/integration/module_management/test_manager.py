...
def sample_target():
    ...
    return TargetModel(
        ...
        endpoints=[{"path": "/api/v1/chat", "method": "POST", "description": "Chat endpoint"}],
    )
...
"""
        ...
        try:
            # Install module
            options = ModuleInstallOptions(force=True, enable_after_install=True)

            result = await manager.install_module(
                external_module_path, options=options, session=mock_session
            )
    ...
    async def test_module_enable_disable_workflow(self, temp_gibson_root, mock_session):
        ...
        mock_session.execute.return_value.scalar_one_or_none.return_value = MagicMock(
            name="integration_test_module", status="enabled"
        )
    ...
    @pytest.mark.asyncio
    async def test_module_execution_integration(
        self, temp_gibson_root, mock_session, sample_target
    ):
        ...
        with patch.object(manager, "_get_module_instance") as mock_get_instance:
            ...
            with patch.object(manager, "get_module", return_value=mock_module_def):
                ...
                result = await manager.execute_module(
                    "integration_test_module", sample_target, context=context, session=mock_session
                )
...
class TestModuleManagerErrorScenarios:
    ...
    async def test_installation_validation_failure(self, temp_gibson_root, mock_session):
        ...
        try:
            ...
            result = await manager.install_module(invalid_module_path, session=mock_session)
