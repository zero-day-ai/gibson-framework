    ...
    async def test_complete_target_workflow(self, db_manager):
        ...
        async with db_manager.get_session() as session:
            ...
            updated = await target_manager.update_target(
                target, description="Updated integration test target", status=TargetStatus.ACTIVE
            )
    ...
    async def test_export_import_workflow(self, db_manager, tmp_path):
        ...
        async with db_manager.get_session() as session:
            ...
            export_count = await target_manager.export_targets(
                export_file, include_credentials=False, filter_kwargs={"environment": "test"}
            )
            ...
            # Import targets back
            import_stats = await target_manager.import_targets(export_file, update_existing=False)
    ...
    async def test_validation_workflow(self, db_manager):
        ...
        async with db_manager.get_session() as session:
            ...
            result = await target_manager.validate_target(
                target.id, test_connection=True, validate_credentials=False
            )
