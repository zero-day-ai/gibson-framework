...
class ModelConverter:
    ...
    def pydantic_to_target(pydantic_target: PydanticTargetModel) -> Optional[TargetRecord]:
        ...
        return TargetRecord(
            ...
            target_type=pydantic_target.target_type.value
            if hasattr(pydantic_target.target_type, "value")
            else pydantic_target.target_type,
            base_url=pydantic_target.base_url,
            status=pydantic_target.status.value
            if hasattr(pydantic_target.status, "value")
            else pydantic_target.status,
            enabled=pydantic_target.enabled,
            authentication=pydantic_target.authentication.model_dump()
            if hasattr(pydantic_target.authentication, "model_dump")
            else pydantic_target.authentication,
            endpoints=[
                e.model_dump() if hasattr(e, "model_dump") else e for e in pydantic_target.endpoints
            ],
    ...
    def scan_to_dict(db_scan: ScanRecord) -> Dict[str, Any]:
        ...
        return {
            ...
            "target_id": str(db_scan.target_id)
            if isinstance(db_scan.target_id, UUID)
            else db_scan.target_id,
    ...
    def finding_to_dict(db_finding: FindingRecord) -> Dict[str, Any]:
        ...
        """
        # Handle tags field - deserialize from JSON if it's a string, otherwise use as-is
        tags = {}
        if db_finding.tags:
            if isinstance(db_finding.tags, str):
                try:
                    tags = json.loads(db_finding.tags)
                except (json.JSONDecodeError, TypeError):
                    tags = {}
            elif isinstance(db_finding.tags, dict):
        ...
        return {
            ...
            "scan_id": str(db_finding.scan_id)
            if isinstance(db_finding.scan_id, UUID)
            else db_finding.scan_id,
            "target_id": str(db_finding.target_id)
            if isinstance(db_finding.target_id, UUID)
            else db_finding.target_id,
    ...
    @staticmethod
    def finding_from_dict(finding_data: Dict[str, Any]) -> Dict[str, Any]:
        """Convert dictionary to format suitable for creating FindingRecord.

        Args:
            finding_data: Dictionary containing finding data

        Returns:
            Dictionary suitable for SQLAlchemy FindingRecord creation
        """
        # Handle tags field - serialize to JSON if it's a dict
        tags_json = None
        if "tags" in finding_data and finding_data["tags"]:
            tags = finding_data["tags"]
            if isinstance(tags, dict):
                tags_json = json.dumps(tags)
            elif isinstance(tags, str):
                # Already JSON string, validate it's proper JSON
                try:
                    json.loads(tags)
                    tags_json = tags
                except (json.JSONDecodeError, TypeError):
        ...
        db_data["tags"] = tags_json

        return db_data

    @staticmethod
    def serialize_tags(tags: Optional[Dict[str, List[str]]]) -> Optional[str]:
        """Serialize tags dict to JSON string for database storage.

        Args:
            tags: Tags dictionary to serialize

        Returns:
            JSON string or None if tags is empty
        """
        if not tags:
            return None
    ...
    @staticmethod
    def deserialize_tags(tags_json: Optional[str]) -> Dict[str, List[str]]:
        """Deserialize tags JSON string to dict.

        Args:
            tags_json: JSON string from database

        Returns:
            Tags dictionary, empty dict if parsing fails
        """
        if not tags_json:
            return {}
