...
def init(
    ...
    with_domains: bool = typer.Option(True, "--domains/--no-domains", help="Include domain settings"),
) -> None:
...
def thresholds(
    ...
    confidence: Optional[int] = typer.Option(None, "--confidence", "-c", help="Confidence threshold"),
) -> None:
    """
...
def export(
    ...
    domain: Optional[str] = typer.Option(None, "--domain", "-d", help="Export specific domain only"),
    include_secrets: bool = typer.Option(False, "--include-secrets", help="Include API keys and secrets"),
) -> None:
...
def _configure_domain(context: Context, domain: str) -> None:
    ...
    timeout = typer.prompt(
        "Timeout in seconds",
        type=int,
        default=domain_settings.get('timeout', 300)
    )
...
def _set_threshold(context: Context, domain: str, severity: Optional[str], confidence: Optional[int]) -> None:
    ...
    try:
        ...
        if confidence is not None:
            ...
            console.print(f"[green]✓[/green] Set confidence threshold for '{domain}' to {confidence}")
