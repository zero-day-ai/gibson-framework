...
def search(
    ...
    category: Optional[str] = typer.Option(None, "--category", "-c", help="Filter by category (official, community, verified)"),
    domain: Optional[str] = typer.Option(None, "--domain", "-d", help="Filter by attack domain (prompts, data, model, system, output)"),
...
def install(
    ...
    version: Optional[str] = typer.Option(None, "--version", "-v", help="Specific version (e.g., >=1.0.0, ==2.1.0)"),
    branch: Optional[str] = typer.Option(None, "--branch", "-b", help="Git branch for Git sources"),
    commit: Optional[str] = typer.Option(None, "--commit", "-c", help="Specific commit hash for Git sources"),
    ...
    dependency_strategy: str = typer.Option(
        "strict", "--dependency-strategy",
        help="Dependency resolution strategy: strict, best_effort, skip"
    ),
    module_name: Optional[str] = typer.Option(None, "--name", help="Specific module name (for Git repos with multiple modules)"),
) -> None:
    """
        ...
        try:
            ...
            if result.success:
                ...
                if result.dependencies_installed:
                    console.print(f"[green]Installed {len(result.dependencies_installed)} dependencies:[/green]")
...
def list(
    ...
    domain: Optional[str] = typer.Option(None, "--domain", "-d", help="Filter by attack domain (prompts, data, model, system, output)"),
    enabled: Optional[bool] = typer.Option(None, "--enabled/--no-enabled", help="Filter by enabled status"),
    ...
    try:
        ...
        async def _list_modules_async():
            if base.db_manager:
                async with base.db_manager.get_session() as session:
                    ...
                    modules_dict = await manager.list_modules(
                        filter=filter_obj,
                        session=session
                    )
        ...
        for data in modules_data:
            ...
            modules.append(Module(
                ...
                installed_date=datetime.fromisoformat(data['installed_date']) if data.get('installed_date') else None,
    ...
    if output == "json":
        ...
        for module in modules:
            ...
            if verbose:
                ...
                module_dict["created"] = module.installed_date.isoformat() if module.installed_date else None
                module_dict["updated"] = module.last_updated.isoformat() if module.last_updated else None
...
def info(
    ...
    module = Module(
        ...
        installed_date=datetime.fromisoformat(module_info['installed_date']) if module_info.get('installed_date') else None,
        last_updated=datetime.fromisoformat(module_info['last_updated']) if module_info.get('last_updated') else None
    )
...
def enable_domain(
    ctx: typer.Context,
    domain: str = typer.Argument(help="Attack domain to enable (prompts, data, model, system, output)"),
) -> None:
    """
...
def disable_domain(
    ctx: typer.Context,
    domain: str = typer.Argument(help="Attack domain to disable (prompts, data, model, system, output)"),
) -> None:
    """
...
def registry(
    ...
    if stats:
        ...
        if registry.metadata:
            ...
            console.print(f"[dim]Last updated: {registry.metadata.last_updated.strftime('%Y-%m-%d %H:%M')}[/dim]")
    ...
    if not any([refresh, stats, export]):
        if registry.metadata:
            ...
            console.print(f"Last updated: {registry.metadata.last_updated.strftime('%Y-%m-%d %H:%M')}")
        else:
