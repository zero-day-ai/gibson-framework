...
from gibson.core.auth.providers import ProviderRegistry, detect_provider_from_key, detect_provider_from_url
...
def add(
    ...
    api_key: Optional[str] = typer.Option(None, "--api-key", "-k", help="API key (will prompt if not provided)"),
    key_format: Optional[str] = typer.Option(None, "--format", "-f", help="Key format: bearer, custom_header, query_param"),
    provider: Optional[str] = typer.Option(None, "--provider", "-p", help="API provider: openai, anthropic, google, etc."),
    environment: Optional[str] = typer.Option("production", "--environment", "-e", help="Environment: production, staging, development"),
    validate: bool = typer.Option(True, "--validate/--no-validate", help="Validate credentials after adding"),
):
    """Add API credentials for a target."""
    asyncio.run(_add_credential_async(target_name, api_key, key_format, provider, environment, validate))
    ...
    try:
        ...
        if not format_enum:
            ...
            if provider == "anthropic":
                ...
                console.print(f"[red]Error: Invalid key format '{key_format}'. Valid options: bearer, custom_header, query_param[/red]")
                raise typer.Exit(1)
...
async def _list_credentials_async(
    ...
    try:
        ...
        for cred in credentials:
            ...
            last_used = cred.last_used_at.strftime("%Y-%m-%d %H:%M") if cred.last_used_at else "Never"
...
def update(
    ...
    validate: bool = typer.Option(True, "--validate/--no-validate", help="Validate new credentials"),
):
...
async def _update_credential_async(target_name: str, api_key: Optional[str], validate: bool):
    ...
    try:
        ...
        if not old_credential:
            console.print(f"[red]Failed to retrieve existing credential for target '{target_name}'[/red]")
            raise typer.Exit(1)
        ...
        console.print(f"[green]✓ Credentials updated successfully for target '{target_name}'[/green]")
...
def export(
    ...
    include_keys: bool = typer.Option(False, "--include-keys", help="Include encrypted API keys (use with caution)"),
):
...
async def _export_credentials_async(output_file: str, format: str, include_keys: bool):
    ...
    try:
        ...
        export_data = {
            "version": "1.0",
            "exported_at": "2025-08-21T13:40:00Z",
            "credentials": []
        }
        ...
        for cred_meta in credentials:
            ...
            if include_keys:
                ...
                if full_cred:
                    ...
                    console.print(f"[yellow]⚠ Including API key for {cred_meta.target_name}[/yellow]")
        ...
        console.print(f"[green]✓ Exported {len(credentials)} credential(s) to {output_file}[/green]")
        
        if include_keys:
            console.print("[red]⚠ WARNING: Exported file contains sensitive API keys. Secure it properly![/red]")
...
def import_credentials(
    ...
    validate: bool = typer.Option(True, "--validate/--no-validate", help="Validate imported credentials"),
...
async def _import_credentials_async(input_file: str, format: str, validate: bool, dry_run: bool):
    ...
    try:
        ...
        for cred_data in credentials_to_import:
            ...
            if dry_run:
                console.print(f"[blue]Would import: {target_name} ({cred_data.get('provider_name', 'unknown')})[/blue]")
                continue
...
def import_env(
    ...
    auto_inject: bool = typer.Option(True, "--auto-inject", help="Automatically inject discovered credentials"),
    ...
    try:
        ...
        injector = EnvironmentCredentialInjector(
            auto_inject=False,  # We'll control injection manually
...
def env_template(
    ctx: typer.Context,
    output_file: Optional[Path] = typer.Option(None, "--output", "-o", help="Output file path (defaults to .env.template)"),
    target_ids: Optional[List[str]] = typer.Option(None, "--target-id", "-t", help="Include specific target IDs in template"),
) -> None:
...
def env_status(
    ...
    try:
        from gibson.core.auth.env_injector import EnvironmentCredentialInjector, detect_ci_environment
