...
from gibson.models.auth import ApiKeyCredentialModel, ApiKeyFormat, ValidationStatus, AuthErrorType
...
def add(
    ...
    description: Optional[str] = typer.Option(
        None, "--description", "-d", help="Credential description"
    ),
    interactive: bool = typer.Option(
        False, "--interactive", "-i", help="Interactive credential setup"
    ),
) -> None:
    """
    ...
    try:
        ...
        if interactive or not api_key:
            ...
            try:
                ...
                console.print(
                    f"[dim]Valid formats: {', '.join([f.value.lower().replace('_', '-') for f in ApiKeyFormat])}[/dim]"
                )
                raise typer.Exit(1)
...
def remove(
    ...
    try:
        ...
        if not force:
            console.print(
                f"[yellow]This will remove the credential for target: {target_id}[/yellow]"
            )
...
def validate(
    ctx: typer.Context,
    target_id: Optional[str] = typer.Argument(
        None, help="Target ID (validate all if not specified)"
    ),
...
def inject_env(
    ...
    try:
        injector = EnvironmentCredentialInjector(auto_inject=False, validate_on_inject=validate)
...
def env_status(
    ...
    try:
        ...
        console.print(
            f"  Auto Inject: {'Enabled' if status['auto_inject_enabled'] else 'Disabled'}"
        )
        console.print(
            f"  Validate on Inject: {'Enabled' if status['validate_on_inject'] else 'Disabled'}"
        )
...
def env_template(
    ...
    target_ids: List[str] = typer.Option(
        None, "--target-id", help="Target IDs to include in template"
    ),
) -> None:
...
def _interactive_credential_setup(
    target_id: UUID, provider: Optional[str]
    ...
    if not provider:
        providers = ["openai", "anthropic", "google", "azure", "custom"]
        provider = Prompt.ask("Provider", choices=providers + [""], default="", show_choices=True)
        if not provider:
            provider = None
    ...
    if provider:
        ...
        format_str = Prompt.ask("Format", choices=format_choices, default="bearer-token")
...
async def _validate_credential(
    credential: ApiKeyCredentialModel, fix_issues: bool = False, quiet: bool = False
) -> bool:
        ...
        if not quiet:
            if result.status == ValidationStatus.VALID:
                ...
                if result.rate_limit_info:
                    console.print(
                        f"[dim]Rate limit: {result.rate_limit_info.requests_remaining} requests remaining[/dim]"
                    )
                ...
                if fix_issues and result.error_type == AuthErrorType.RATE_LIMITED:
                    console.print(
                        "[yellow]Credential is rate limited, consider waiting before retrying[/yellow]"
                    )
