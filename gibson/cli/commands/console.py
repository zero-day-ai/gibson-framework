    ...
    async def run_scan():
        ...
        try:
            result = await scanner.scan(
                target=target, scan_type=scan_type, dry_run=True
            )  # Safe default
            ...
            if result.findings:
                ...
                for finding in result.findings[:5]:  # Show first 5
                    ...
                    console.print(
                        f"  • [{severity_color}]{finding.severity}[/{severity_color}] {finding.title}"
                    )
    ...
    async def run_module_command():
        ...
        try:
            if action == "list":
                ...
                console.print(
                    "[red]Usage: module <list|search <query>|install <name>|enable <name>|disable <name>>[/red]"
                )
    ...
    try:
        ...
        try:
            ...
            if action == "list":
                ...
                console.print(f"[bold]Search Results for '{query}':[/bold]")
                console.print(
                    "  • advanced-prompt-injection v2.0.0 - Advanced prompt injection tests"
                )
            elif action == "install":
...
def _handle_research(args: list[str], context: Context, console: Console) -> None:
    ...
    if matching_topic:
        ...
        console.print(
            f"  Suggestion: Try researching 'prompt injection', 'owasp llm', or 'ai security'\n"
        )
