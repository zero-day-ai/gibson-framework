...
def list_chains(
    ...
    if format == "table":
        ...
        for chain in sorted(chains, key=lambda x: x.name):
            created_str = chain.created_at.strftime("%Y-%m-%d") if hasattr(chain.created_at, 'strftime') else str(chain.created_at)[:10]
...
def run(
    ...
    parallel: bool = typer.Option(False, "--parallel", "-p", help="Enable parallel execution where possible"),
    ...
    try:
        # Execute chain
        results = asyncio.run(
            _execute_chain(chain, target, parallel or chain.parallel_execution)
        )
...
def _get_modules_for_domain(domain: str) -> List[str]:
    ...
    domain_modules = {
        "llm-prompt-injection": [
            "prompt_injection", "jailbreak_detection", "indirect_injection",
            "system_prompt_leakage", "role_playing_bypass"
        ],
        "llm-model-theft": [
            "model_theft", "api_extraction", "weight_analysis", "query_based_extraction"
        ],
        "llm-sensitive-info": [
            "sensitive_info_disclosure", "data_extraction", "privacy_leakage",
...
def _validate_chain(chain: AttackChain) -> Dict[str, Any]:
    ...
    available_modules = ["prompt_injection", "model_theft", "target_analysis"]  # This would be dynamic
    for step in chain.steps:
