...
class CLIErrorHandler:
    ...
    def handle_exception(
        ...
        error_response = ErrorResponse.from_exception(
            exception, command=command, include_traceback=self.debug
        )
    ...
    def display_error(self, error: ErrorResponse) -> None:
        ...
        self.console.print(
            Panel(
                "\n".join(
                    str(item) if isinstance(item, str) else "" for item in panel_content if item
                ),
...
class ValidationErrorFormatter:
    ...
    def format_error(error: ValidationError, model_name: str = "Input") -> str:
        ...
        if len(error.errors()) == 1:
            return ValidationErrorFormatter._format_single_error(error.errors()[0], model_name)

        return ValidationErrorFormatter._format_multiple_errors(error.errors(), model_name)
    ...
    @staticmethod
    def _format_multiple_errors(errors: List[Dict[str, Any]], model_name: str) -> str:
...
class ContextualErrorHandler:
    ...
    def _handle_scan_errors(self, error: Exception, data: Dict[str, Any]) -> List[str]:
    ...
    def _handle_module_errors(self, error: Exception, data: Dict[str, Any]) -> List[str]:
    ...
    def _handle_target_errors(self, error: Exception, data: Dict[str, Any]) -> List[str]:
    ...
    def _handle_config_errors(self, error: Exception, data: Dict[str, Any]) -> List[str]:
...
def create_error_response(
    error: Exception, command: Optional[str] = None, include_traceback: bool = False
) -> ErrorResponse:
    """Create standardized error response from exception."""
    return ErrorResponse.from_exception(error, command=command, include_traceback=include_traceback)
