            ...
            for item in data:
                ...
                for col in columns:
                    ...
                    if col == 'severity' and value in self.severity_colors:
                        styled_value = Text(str(value).upper(), 
                                          style=self.severity_colors[value])
    ...
    def _dict_to_table(
        ...
        for key, value in data_dict.items():
            ...
            if key == 'severity' and value in self.severity_colors:
                styled_value = Text(value_str.upper(), 
                                  style=self.severity_colors[value])
    ...
    def _to_sarif_result(self, item_dict: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "locations": [{
                "physicalLocation": {
                    "artifactLocation": {
                        "uri": item_dict.get('target', 'unknown')
                    }
                }
            }],
            "properties": {
        ...
        **kwargs
        ...
        if isinstance(data, list) and data:
            ...
            if hasattr(first_item, 'model_dump'):
                ...
                # Table header
                output.append("| " + " | ".join(h.replace('_', ' ').title() for h in headers) + " |")
