...
class SecurityIssue(GibsonBaseModel):
    ...
    location: Optional[str] = Field(
        default=None,
        description="Location in code where issue was found (e.g., 'line 42')"
    )
...
class ValidationResult(GibsonBaseModel):
    ...
    valid: bool = Field(
        description="Whether the module passed validation"
    )
    errors: List[str] = Field(
        default_factory=list,
        description="List of validation errors that prevent module use"
    )
    ...
    security_issues: List[SecurityIssue] = Field(
        default_factory=list,
        description="List of security issues found in the module"
    )
    required_permissions: List[str] = Field(
        default_factory=list,
        description="List of permissions required by this module"
    )
    ...
    validator_version: Optional[str] = Field(
        default=None,
        description="Version of the validation system used"
    )
...
class ModuleInstallOptions(GibsonBaseModel):
    ...
    force: bool = Field(
        default=False,
        description="Force overwrite if module already exists"
    )
    dev_mode: bool = Field(
        default=False,
        description="Install in development mode (editable install)"
    )
    ...
    verify_signature: bool = Field(
        default=True,
        description="Verify module signature if available"
    )
    ...
    backup_existing: bool = Field(
        default=True,
        description="Create backup of existing module before overwrite"
    )
    install_docs: bool = Field(
        default=False,
        description="Install module documentation"
    )
    enable_after_install: bool = Field(
        default=True,
        description="Enable module after successful installation"
    )
...
class DependencyNode(GibsonBaseModel):
    ...
    version_spec: str = Field(
        default="*",
        description="Version specification (e.g., '>=1.0.0')"
    )
    ...
    dependencies: List[str] = Field(
        default_factory=list,
        description="List of this node's dependencies"
    )
...
class DependencyEdge(GibsonBaseModel):
    ...
    version_constraint: str = Field(
        default="*",
        description="Version constraint for this dependency"
    )
    is_dev: bool = Field(
        default=False,
        description="Whether this is a development dependency"
...
class DependencyConflict(GibsonBaseModel):
    ...
    package: str = Field(
        description="Name of conflicting package"
    )
    requirements: List[str] = Field(
        description="List of conflicting requirements"
    )
...
class DependencyResolutionResult(GibsonBaseModel):
    ...
    resolution_order: List[str] = Field(
        default_factory=list,
        description="Order in which dependencies should be installed"
    )
    python_packages: List[str] = Field(
        default_factory=list,
        description="Python packages to install"
    )
    gibson_modules: List[str] = Field(
        default_factory=list,
        description="Gibson modules to install"
    )
    conflicts: List[DependencyConflict] = Field(
        default_factory=list,
        description="Any conflicts found"
    )
    warnings: List[str] = Field(
        default_factory=list,
...
class InstallationResult(GibsonBaseModel):
    ...
    install_path: Path = Field(
        description="Path where module was installed"
    )
    source: str = Field(
        description="Source from which module was installed"
    )
    ...
    warnings: List[str] = Field(
        default_factory=list,
        description="Any warnings during installation"
    )
    error: Optional[str] = Field(
        default=None,
        description="Error message if installation failed"
    )
    metadata: Optional[ModuleDefinitionModel] = Field(
        default=None,
...
class ModuleUpdateInfo(GibsonBaseModel):
    ...
    breaking_changes: bool = Field(
        default=False,
        description="Whether the update contains breaking changes"
    )
    update_size: int = Field(
        default=0,
        ge=0,
    ...
    security_fixes: List[str] = Field(
        default_factory=list,
        description="List of security issues fixed in this update"
    )
    ...
    def requires_attention(self) -> bool:
        ...
        return (
            ...
            self.dependencies_changed or 
            self.update_priority in ['high', 'critical'] or
            bool(self.security_fixes)
        )
...
class ModuleExecutionContextModel(GibsonBaseModel):
    ...
    module_name: str = Field(
        description="Name of module being executed"
    )
    ...
    resource_limits: Dict[str, Any] = Field(
        default_factory=dict,
        description="Resource limits (memory, CPU, time, etc.)"
    )
    timeout: int = Field(
        default=300,
        gt=0,
...
class ModuleFilter(GibsonBaseModel):
    ...
    domains: Optional[List[AttackDomain]] = Field(
        default=None,
        description="Filter by attack domains"
    )
    ...
    min_version: Optional[str] = Field(
        default=None,
        description="Minimum version constraint"
    )
    severity: Optional[List[Severity]] = Field(
        default=None,
        description="Filter by risk severity levels"
    )
    ...
    enabled_only: bool = Field(
        default=False,
        description="Only return enabled modules"
    )
    has_updates: bool = Field(
        default=False,
        description="Only return modules with available updates"
    )
    installed_after: Optional[datetime] = Field(
        default=None,
        description="Filter by installation date"
    )
    ...
    sort_by: str = Field(
        default="name",
        description="Sort field (name, version, installed_date, etc.)"
    )
    ...
    def validate_sort_field(cls, v: str) -> str:
        """Validate sort field."""
        allowed = [
            'name', 'version', 'author', 'installed_date', 
            'last_updated', 'usage_count', 'domain'
        ]
        if v not in allowed:
...
class InstallationResult(GibsonBaseModel):
    ...
    module_name: str = Field(
        description="Name of installed module"
    )
    ...
    installed_modules: List[str] = Field(
        default_factory=list,
        description="List of modules successfully installed"
    )
    skipped_modules: List[str] = Field(
        default_factory=list,
        description="List of modules that were skipped"
    )
    failed_modules: List[str] = Field(
        default_factory=list,
        description="List of modules that failed to install"
    )
    dependency_failures: List[str] = Field(
        default_factory=list,
        description="List of dependencies that failed to install"
    )
    errors: List[str] = Field(
        default_factory=list,
    ...
    installation_time: float = Field(
        default=0.0,
        ge=0,
        description="Total installation time in seconds"
    )
    backup_path: Optional[Path] = Field(
        default=None,
        description="Path to backup of previous version"
    )
