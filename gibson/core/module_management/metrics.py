...
class PerformanceMetrics:
    ...
    def __init__(self, window_size: int = 100):
        ...
        self.execution_times: Dict[str, deque] = defaultdict(lambda: deque(maxlen=window_size))
        self.success_counts: Dict[str, int] = defaultdict(int)
        self.failure_counts: Dict[str, int] = defaultdict(int)
        self.timeout_counts: Dict[str, int] = defaultdict(int)
        self.finding_counts: Dict[str, List[int]] = defaultdict(list)
        self.resource_usage: Dict[str, deque] = defaultdict(lambda: deque(maxlen=window_size))
        self.last_execution: Dict[str, datetime] = {}
        self.total_executions: Dict[str, int] = defaultdict(int)

    def record_execution(self, module_name: str, result: ModuleResultModel) -> None:
        """
    ...
    def get_global_stats(self) -> Dict[str, Any]:
        ...
        if total_executions == 0:
            return {"total_executions": 0, "global_success_rate": 0.0, "module_count": 0}
        ...
        return {
            ...
            "findings_per_execution": round(sum(all_findings) / total_executions, 2)
            if total_executions
            else 0.0,
        }

    def get_trending_metrics(self, module_name: str, period_hours: int = 24) -> Dict[str, Any]:
        """
    ...
    def identify_problematic_modules(
        self, failure_threshold: float = 0.3, timeout_threshold: float = 0.2
    ) -> List[Dict[str, Any]]:
        """
...
class MetricsCollector:
    ...
    def __init__(
        self, metrics_dir: Optional[Path] = None, persist_interval: int = 300  # 5 minutes
    ):
        """
    ...
    def record_execution(
        self, module_name: str, result: ModuleResultModel, target_info: Optional[Dict] = None
    ) -> None:
        """
    ...
    def persist_metrics(self) -> None:
        ...
        try:
            ...
            metrics_data = {"last_updated": datetime.utcnow().isoformat(), "modules": {}}

            for module_name in self.performance_metrics.total_executions:
                metrics_data["modules"][module_name] = self.performance_metrics.get_module_stats(
                    module_name
                )
            ...
            log_file = (
                self.metrics_dir / f"execution_log_{datetime.utcnow().strftime('%Y%m%d')}.jsonl"
            )
    ...
    def generate_report(
        self, module_name: Optional[str] = None, period_days: int = 7
    ) -> Dict[str, Any]:
        """
        ...
        if module_name:
            ...
            trending = self.performance_metrics.get_trending_metrics(module_name, period_days * 24)
