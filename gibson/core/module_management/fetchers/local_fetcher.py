...
class LocalFetcher(BaseFetcher):
    ...
    async def fetch(
        ...
        if not source_path.exists():
            raise ModuleInstallationError(f"Local source not found: {source_path}")
        ...
        try:
            if source_path.is_dir():
                # Copy directory
                module_path = await self._copy_directory(source_path, target_dir, options)
            elif source_path.is_file():
                # Handle archive files
                if self._is_archive(source_path):
                    module_path = await self._extract_archive(source_path, target_dir)
                else:
    ...
    async def _copy_directory(
        self, source_path: Path, target_dir: Path, options: Dict[str, Any]
    ) -> Path:
        ...
        if target_path.exists():
            if options.get("overwrite", True):
                ...
                raise ModuleInstallationError(f"Target already exists: {target_path}")
        ...
        if options.get("symlink", False):
            ...
            shutil.copytree(
                ...
                ignore=shutil.ignore_patterns(
                    "*.pyc", "__pycache__", ".git", ".pytest_cache", "*.egg-info"
                ),
            )
    ...
    def _is_archive(self, path: Path) -> bool:
        ...
        supported_extensions = {".tar", ".tar.gz", ".tgz", ".zip", ".tar.bz2", ".tbz2"}
