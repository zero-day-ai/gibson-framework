...
class RegistryFetcher(BaseFetcher):
    ...
    async def fetch(
        ...
        try:
            ...
            if not metadata:
                raise ModuleInstallationError(f"Module '{module_name}' not found in registry")

            # Download module archive
            download_url = metadata.get("download_url")
            if not download_url:
                raise ModuleInstallationError(f"No download URL for module '{module_name}'")

            archive_path = await self._download_module(
                download_url, target_dir, module_name, options
            )
    ...
    async def _get_module_metadata(
        self, module_name: str, version: str, options: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """Get module metadata from registry."""
        session = await self._get_session()

        # Build metadata URL
        metadata_url = urljoin(self.registry_url, f"/modules/{module_name}/versions/{version}")
        ...
        try:
            async with session.get(metadata_url, headers=headers) as response:
                if response.status == 200:
                    ...
                    raise ModuleInstallationError(f"Registry returned {response.status}: {text}")
        except aiohttp.ClientError as e:
            raise ModuleInstallationError(f"Registry connection failed: {e}")

    async def _download_module(
        self, download_url: str, target_dir: Path, module_name: str, options: Dict[str, Any]
    ) -> Path:
        ...
        try:
            async with session.get(download_url, headers=headers) as response:
                if response.status != 200:
                    text = await response.text()
                    raise ModuleInstallationError(f"Download failed ({response.status}): {text}")
    ...
    async def _extract_archive(self, archive_path: Path, target_dir: Path) -> Path:
        ...
        try:
            if archive_path.suffix == ".gz" and archive_path.stem.endswith(".tar"):
                ...
                raise ModuleInstallationError(f"Unsupported archive format: {archive_path.suffix}")
