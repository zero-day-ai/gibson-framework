...
class DependencyError(ModuleManagementError):
    ...
    def __init__(
        ...
        if circular_deps:
            suggestions.append(f"Circular dependency detected: {' -> '.join(circular_deps)}")
        ...
        if conflicts:
            ...
            for conflict in conflicts[:3]:  # Show first 3 conflicts
                ...
                suggestions.append(f"  {dep_name}: required {required}, installed {installed}")
...
class CircularDependencyError(DependencyError):
    ...
    def __init__(self, message: str, circular_path: List[str], module_name: Optional[str] = None):
        super().__init__(message, module_name=module_name, circular_deps=circular_path)
...
class DependencyConflictError(DependencyError):
    ...
    def __init__(
        self, message: str, conflicts: List[Dict[str, Any]], module_name: Optional[str] = None
    ):
        super().__init__(message, module_name=module_name, conflicts=conflicts)
