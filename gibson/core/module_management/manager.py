...
class ModuleManager:
    ...
    def __init__(self, config_path: Optional[Path] = None, modules_dir: Optional[Path] = None):
        ...
        self.modules_dir = (
            modules_dir
            or Path(self.config["module_management"]["installation"]["base_dir"]).expanduser()
        )
        ...
        self.metrics_collector = MetricsCollector(
            metrics_dir=Path(
                self.config["module_management"]["metrics"]["metrics_dir"]
            ).expanduser(),
        ...
        if config_path and config_path.exists():
            ...
            return {
                "module_management": {
                    ...
                    "registry": {"url": "https://registry.gibson.ai/api/v1"},
                    "cache": {"directory": "~/.gibson/cache"},
                    "metrics": {"metrics_dir": "~/.gibson/metrics", "persist_interval": 300},
                    "discovery": {
                        "auto_discover": True,
                        "discovery_interval": 3600,
                        "scan_paths": ["~/.gibson/modules", "./gibson/core/modules"],
                    },
                }
            }
    ...
    async def install(
        self, source: str, options: Optional[ModuleInstallOptions] = None
    ) -> InstallationResult:
        """
        ...
        if result.success:
            ...
            if result.module_name:
                self.version_manager.record_version(
                    result.module_name, result.installed_version or "unknown"
                )
    ...
    async def uninstall(self, module_name: str) -> bool:
        ...
        if module_dir.exists():
            ...
            backup_dir = Path(
                self.config["module_management"]["installation"]["backup_dir"]
            ).expanduser()
            backup_path = (
                backup_dir / f"{module_name}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    async def execute(
        self, module_name: str, target: TargetModel, parameters: Optional[Dict[str, Any]] = None
    ) -> ModuleResultModel:
        """
        Execute a module.
        ...
        # Execute module
        result = await self.executor.execute(module_name, context, self._module_cache[module_name])

        # Record metrics
        self.metrics_collector.record_execution(
            module_name, result, {"target": target.model_dump()}
        )
    ...
    async def update_module(
        self, module_name: str, target_version: Optional[str] = None
    ) -> InstallationResult:
        """
        ...
        if not update_info and not target_version:
            return InstallationResult(
                success=False, module_name=module_name, error_message="No updates available"
            )
        ...
        options = ModuleInstallOptions(force_overwrite=True, backup_existing=True)

        return await self.install(source, options)

    async def resolve_dependencies(self, module_name: str) -> DependencyResolutionResult:
        """
        ...
        return await self.dependency_resolver.resolve_dependencies(
            module_name, dependencies, dev_dependencies
        )

    async def get_metrics_report(self, module_name: Optional[str] = None) -> Dict[str, Any]:
        """
        Get metrics report.
