...
class DependencyResolver:
    ...
    async def resolve_dependencies(
        ...
        try:
            ...
            graph = await self._build_dependency_graph(module_name, dependencies, dev_dependencies)
            ...
            if conflicts and not await self._resolve_conflicts(conflicts):
                raise VersionConflictError(f"Cannot resolve version conflicts: {conflicts}")
            ...
            return DependencyResolutionResult(
                success=False,
                graph=DependencyGraph(root_module=module_name, nodes={}, edges=[], conflicts=[]),
                resolution_order=[],
                python_packages=[],
                gibson_modules=[],
                conflicts=[],
    ...
    async def _build_dependency_graph(
        ...
        return DependencyGraph(root_module=root_module, nodes=nodes, edges=edges, conflicts=[])
    ...
    async def _find_version_conflicts(self, graph: DependencyGraph) -> List[DependencyConflict]:
        ...
        for edge in graph.edges:
            version_requirements[edge.target].append(
                {"source": edge.source, "constraint": edge.version_constraint}
            )
        ...
        for package, requirements in version_requirements.items():
            ...
            if specifiers and not self._are_specifiers_compatible(specifiers):
                conflict = DependencyConflict(
                    package=package,
                    requirements=[
                        f"{r['source']} requires {r['constraint']}" for r in requirements
                    ],
                    resolution=None,
                )
    ...
    def _get_known_gibson_modules(self) -> Set[str]:
        ...
        return {"gibson-prompts", "gibson-data", "gibson-model", "gibson-system", "gibson-output"}

    async def check_compatibility(
        self, module_name: str, target_version: str, current_dependencies: Dict[str, str]
    ) -> bool:
        """
    ...
    async def get_dependency_tree(self, module_name: str, max_depth: int = 10) -> Dict[str, Any]:
        """
        ...
        """
        tree = {"name": module_name, "version": "*", "dependencies": []}
        ...
        async def build_tree(node: Dict, current_module: str, depth: int):
            ...
            for dep_spec in deps:
                dep_name, version_spec = self._parse_dependency_spec(dep_spec)
                child = {"name": dep_name, "version": version_spec, "dependencies": []}
