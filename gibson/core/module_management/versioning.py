...
class VersionManager:
    ...
    def is_compatible(self, current_version: str, required_version: str) -> bool:
        """
        ...
        try:
            ...
            if required_version.startswith("^"):
                ...
                base = self.parse_version(required_version[1:])
                return (
                    current >= base and current.major == base.major and current.minor == base.minor
                )
            else:
                # Standard specifier
    ...
    async def check_for_updates(
        self, installed_modules: Dict[str, ModuleDefinitionModel]
        ...
        for module_name, module_def in installed_modules.items():
            ...
            if latest_version and self.compare_versions(module_def.version, latest_version) < 0:
                # Update available
                changelog = await self.get_changelog(
                    module_name, module_def.version, latest_version
                )
                ...
                update_info = ModuleUpdateInfo(
                    ...
                    changelog=changelog,
                    breaking_changes=self._has_breaking_changes(module_def.version, latest_version),
    ...
    def _has_breaking_changes(self, current_version: str, target_version: str) -> bool:
        """
    ...
    def get_version_history(self, module_name: str) -> List[str]:
        """
    ...
    def record_version(self, module_name: str, version: str) -> None:
        """
        ...
        data["installations"].append(
            {"version": version, "timestamp": datetime.utcnow().isoformat()}
        )
    ...
    async def get_version_metadata(self, module_name: str, version: str) -> Optional[Dict]:
        """
    ...
    def suggest_upgrade_path(
        self, module_name: str, current_version: str, target_version: Optional[str] = None
    ) -> List[str]:
        """
        ...
        upgrade_candidates = [v for v in history if self.parse_version(v) > current]
        ...
        if target_version:
            ...
            upgrade_candidates = [v for v in upgrade_candidates if self.parse_version(v) <= target]
        ...
        for v_str in upgrade_candidates:
            ...
            if hasattr(v, "major") and v.major > last_major:
                ...
                prev_major_versions = [
                    ...
                    if hasattr(self.parse_version(c), "major")
                    and self.parse_version(c).major == last_major
                ]
