...
class ModuleValidator:
        ...
        try:
            for dep in deps:
                ...
                if self._is_dangerous_dependency(dep):
                    result.add_security_issue(SecurityIssue(
    ...
    def _find_module_classes(
        ...
        for name, obj in inspect.getmembers(module, inspect.isclass):
            if (issubclass(obj, expected_base) and 
                obj is not expected_base and
    ...
    def _check_import_safety(self, import_name: str, result: ValidationResult) -> None:
        ...
        suspicious_patterns = [
            'ctypes', 'ctypes.util', 'win32api', 'win32con',
        ...
        for pattern in suspicious_patterns:
            if pattern in import_name:
                result.add_warning(
                    f"Import requires review for security: {import_name}"
                )
    ...
    def _analyze_code_complexity(self, tree: ast.AST, result: ValidationResult) -> None:
        ...
        if complexity_visitor.nesting_depth > 6:
            result.add_warning(
                f"Deep nesting detected: {complexity_visitor.nesting_depth} levels"
            )
