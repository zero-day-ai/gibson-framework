...
from gibson.core.module_management.fetchers import GitFetcher, RegistryFetcher, LocalFetcher
...
class ModuleInstaller:
        ...
        try:
            ...
            if not fetcher:
                raise ModuleInstallationError(
                    f"No suitable fetcher found for source: {source}", source=source
                )

            logger.info(f"Installing module from {source}")

            # Fetch module to temp directory
            module_path = await fetcher.fetch(source, temp_dir, options.model_dump())
            ...
            if not options.skip_deps and metadata.dependencies:
                ...
                if dependency_result.python_packages:
                    logger.info(
                        f"Installing {len(dependency_result.python_packages)} Python dependencies"
                    )
                    install_results = await self.pip_client.install_packages(
                        dependency_result.python_packages, continue_on_error=options.force
                    )
    ...
    async def uninstall_module(self, module_name: str, remove_deps: bool = False) -> bool:
        """
        Uninstall a module.
        ...
        if not module_path.exists():
            raise ModuleInstallationError(
                f"Module '{module_name}' not found", module_name=module_name
            )
