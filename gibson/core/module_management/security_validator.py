...
class SecurityValidator:
    ...
    async def _check_obfuscation(
        ...
        concat_count = len([n for n in ast.walk(tree) if isinstance(n, ast.BinOp) and isinstance(n.op, ast.Add)])
        if concat_count > 20:
            obfuscation_score += 1
    ...
    async def _analyze_code_injection(
        self, 
        tree: ast.AST, 
        result: ValidationResult
    ) -> None:
    ...
    async def _check_data_exfiltration(
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                ...
                if any(pattern in call_name for pattern in ['requests.post', 'httpx.post', 'socket.send']):
                    result.add_security_issue(SecurityIssue(
