...
class PayloadFetcher:
    ...
    async def sync_repository(
        ...
        repository: str,
        branch: str = "main",
        target_domains: Optional[List[AttackDomain]] = None,
        ...
        result = SyncResult(
            success=False, repository=repository, branch=branch, sync_timestamp=start_time
        )
        try:
            ...
            if not repo_info:
                result.errors.append(f"Repository {repository} not found or inaccessible")
                return result
            latest_commit = await self._get_latest_commit(repository, branch)
            if not latest_commit:
                result.errors.append(f"Branch {branch} not found in {repository}")
                return result
            result.last_commit = latest_commit["sha"]
            tree_items = await self._get_repository_tree(repository, latest_commit["sha"])
            if not tree_items:
                result.errors.append("Failed to retrieve repository contents")
                return result
            payload_files = self._filter_payload_files(tree_items, target_domains)
            ...
            for file_info in payload_files:
                try:
                    payload_data = await self._download_file_content(repository, file_info)
                    if payload_data:
                        download_size += len(payload_data)
                        payload = self._parse_payload_file(file_info, payload_data, repository)
                        if payload:
                            result.new_payloads.append(payload.name)
                            result.imported_count += 1
                        else:
                            result.errors.append(f"Failed to parse {file_info['path']}")
            ...
            result.sync_duration_ms = int((end_time - start_time).total_seconds() * 1000)
            logger.info(
    ...
    async def _get_repository_info(self, repository: str) -> Optional[Dict[str, Any]]:
    ...
    async def _get_latest_commit(self, repository: str, branch: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to get latest commit for {repository}:{branch}: {e}")
            return None

    async def _get_repository_tree(
        self, repository: str, commit_sha: str, recursive: bool = True
    ) -> List[Dict[str, Any]]:
        ...
        """
        try:
            url = f"https://api.github.com/repos/{repository}/git/trees/{commit_sha}"
    ...
    def _filter_payload_files(
        self, tree_items: List[Dict[str, Any]], target_domains: Optional[List[AttackDomain]] = None
    ) -> List[Dict[str, Any]]:
        ...
        domain_paths = {
            AttackDomain.PROMPT: ["prompts", "prompt", "injection"],
            AttackDomain.DATA: ["data", "datasets", "poisoning"],
            AttackDomain.MODEL: ["model", "models", "adversarial"],
            AttackDomain.SYSTEM: ["system", "enum", "enumeration"],
            AttackDomain.OUTPUT: ["output", "response", "format"],
        }
        for item in tree_items:
            ...
            if target_domains and item_domain not in target_domains:
                continue
            if any(skip in path.name.lower() for skip in ["readme", "license", "changelog"]):
                continue
        ...
        logger.debug(f"Filtered {len(payload_files)} payload files from {len(tree_items)} items")
        return payload_files

    def _determine_domain_from_path(
        self, path: Path, domain_paths: Dict[AttackDomain, List[str]]
    ) -> Optional[AttackDomain]:
    ...
    def _determine_attack_type_from_path(self, path: Path) -> str:
        ...
        path_str = str(path).lower()
        attack_patterns = {
            "injection": ["inject", "sqli", "xss", "command"],
            "jailbreak": ["jailbreak", "bypass", "escape"],
            "poisoning": ["poison", "backdoor", "trojan"],
            "evasion": ["evasion", "adversarial", "dodge"],
            "enumeration": ["enum", "scan", "discover"],
            "extraction": ["extract", "leak", "dump"],
        }
    ...
    async def _download_file_content(
        self, repository: str, file_info: Dict[str, Any]
    ) -> Optional[str]:
        ...
        try:
            if file_info.get("size", 0) < 1024 * 1024:
                url = f"https://api.github.com/repos/{repository}/contents/{file_info['path']}"
                response = await self._session.get(url)
                response.raise_for_status()
            ...
            url = f"https://api.github.com/repos/{repository}/git/blobs/{file_info['sha']}"
            response = await self._session.get(url)
            response.raise_for_status()
    ...
    def _parse_payload_file(
        self, file_info: Dict[str, Any], content: str, repository: str
    ...
    def _parse_json_payload(
        ...
        try:
            ...
            if isinstance(data, dict) and "payload" in data:
                return PayloadModel.from_minimal(
                    name=data.get("name", Path(file_info["path"]).stem),
                    content=data["payload"],
                    domain=file_info.get("domain", AttackDomain.PROMPT),
                    attack_type=data.get("attack_type", file_info.get("attack_type", "generic")),
    ...
    def _parse_text_payload(
        ...
        try:
            ...
            return PayloadModel.from_minimal(
                name=metadata.get("name", Path(file_info["path"]).stem),
                content=payload_content,
                domain=file_info.get("domain", AttackDomain.PROMPT),
                attack_type=metadata.get("attack_type", file_info.get("attack_type", "generic")),
            ...
            logger.error(f"Failed to parse text payload {file_info['path']}: {e}")
            return None

    async def list_repository_payloads(
    ...
    async def check_repository_access(self, repository: str) -> Tuple[bool, Optional[str]]:
