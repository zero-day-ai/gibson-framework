...
class PayloadOrganizer:
    ...
    def generate_file_path(self, payload: PayloadModel, extension: str = ".txt") -> Path:
        ...
        category_str = (
            payload.category if isinstance(payload.category, str) else payload.category.value
        )
    ...
    def list_payload_files(
        self, domain: Optional[Union[str, AttackDomain]] = None, attack_type: Optional[str] = None
    ) -> List[Path]:
        ...
        try:
            ...
            for file_path in all_files:
                if file_path.exists():
                    ...
                    file_info.append({"path": file_path, "size": size, "mtime": mtime})
            ...
            stats["largest_files"] = [
                {"path": str(f["path"]), "size_bytes": f["size"]} for f in file_info[:10]
            ]

            # Get oldest and newest files
            file_info.sort(key=lambda x: x["mtime"])
            stats["oldest_files"] = [
                {"path": str(f["path"]), "modified": f["mtime"]} for f in file_info[:5]
            ]
            ...
            for root, dirs, files in os.walk(self.base_path, topdown=False):
                ...
                try:
                    if not any(root_path.iterdir()):
                        ...
                        if root_path.parent != self.base_path or root_path.name not in [
                            d.value for d in AttackDomain
                        ]:
                            root_path.rmdir()
                            removed_count += 1
    ...
    async def move_payload_file(self, old_path: Path, new_payload: PayloadModel) -> Optional[Path]:
