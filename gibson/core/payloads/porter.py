...
class PayloadPorter:
    ...
    async def _export_archive(self, bundle: PayloadBundle, output_path: Path) -> Dict[str, Any]:
        ...
        compression = "gz" if output_path.suffix == ".gz" else "bz2" if output_path.suffix == ".bz2" else None
    ...
    async def _import_auto_detect(self, input_path: Path) -> PayloadBundle:
        ...
        try:
            ...
            if content.startswith("{") or content.startswith("payload:"):
                ...
                try:
                    payload_data = json.loads(content) if content.startswith("{") else yaml.safe_load(content)
    ...
    def _detect_format(self, path: Path) -> str:
        ...
        if suffix in {".tar", ".tgz"} or path.name.endswith(".tar.gz") or path.name.endswith(".tar.bz2"):
            return "archive"
        elif suffix == ".json":
            return "json"
