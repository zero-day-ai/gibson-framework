...
class ValidationResult:
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'is_valid': self.is_valid, 'errors': self.errors,
            'warnings': self.warnings, 'quality_score': self.quality_score,
            'suggestions': self.suggestions, 'metadata': self.metadata}
...
class PayloadValidator:
    ...
    def validate_batch(self, payloads: List[PayloadModel]) ->Dict[str,
        ValidationResult]:
    ...
    def get_validation_summary(self, results: Dict[str, ValidationResult]
        ) ->Dict[str, Any]:
        ...
        quality_scores = [r.quality_score for r in results.values() if r.
            quality_score > 0]
        avg_quality = sum(quality_scores) / len(quality_scores
            ) if quality_scores else 0
        return {'total_payloads': total, 'valid_payloads': valid,
            'invalid_payloads': invalid, 'validation_rate': valid / total *
            100 if total > 0 else 0, 'total_errors': total_errors,
            'total_warnings': total_warnings, 'average_quality_score':
            round(avg_quality, 2), 'quality_distribution': self.
            _get_quality_distribution(quality_scores)}

    def _validate_structure(self, payload: PayloadModel, result: ValidationResult
        ) ->None:
        ...
        try:
            ...
            for error in e.errors():
                result.add_error(
                    f"Structure error: {error['msg']} in {error['loc']}")
    ...
    def _validate_content(self, payload: PayloadModel, result: ValidationResult
        ) ->None:
        ...
        try:
            ...
            result.add_error(
                'Payload content contains invalid Unicode characters')
    ...
    def _validate_metadata(self, payload: PayloadModel, result: ValidationResult
        ) ->None:
        """Validate payload metadata."""
        if payload.version and not re.match('^\\d+\\.\\d+\\.\\d+$', payload
            .version):
        ...
        if payload.tags:
            for tag in payload.tags:
                if not isinstance(tag, str):
                    ...
                    result.add_warning(
                        f'Tag contains special characters: {tag}')
    ...
    def _validate_domain_specific(self, payload: PayloadModel, result:
    ...
    def _validate_security(self, payload: PayloadModel, result: ValidationResult
        ) ->None:
        ...
        if payload.attack_vector == AttackVector.INJECTION:
            if not any(keyword in content for keyword in ['select',
                'insert', 'delete', 'union', '<script']):
                result.add_suggestion(
                    'Injection payload should contain injection techniques')

    def _validate_hash(self, payload: PayloadModel, result: ValidationResult
        ) ->None:
    ...
    def _assess_quality(self, payload: PayloadModel, result: ValidationResult
        ) ->None:
        ...
        if result.quality_score < 50:
            result.add_suggestion(
                'Consider adding more descriptive documentation')
    ...
    def _validate_prompts_domain(self, payload: PayloadModel, result:
        ValidationResult) ->None:
        """Validate prompts domain requirements."""
        valid_attack_types = ['injection', 'jailbreak', 'context_steering',
            'role_play', 'instruction_bypass', 'token_smuggling']
        if payload.attack_type not in valid_attack_types:
            result.add_warning(
                f'Unusual attack type for prompts domain: {payload.attack_type}'
                )
        content = payload.content.lower()
        if (payload.attack_type == 'injection' and 'ignore' not in content and
            'bypass' not in content):
            result.add_suggestion(
                "Injection prompts often use 'ignore' or 'bypass' instructions"
                )

    def _validate_data_domain(self, payload: PayloadModel, result: ValidationResult
        ) ->None:
        """Validate data domain requirements."""
        valid_attack_types = ['poisoning', 'backdoor',
            'membership_inference', 'extraction', 'reconstruction', 'inversion'
            ]
        if payload.attack_type not in valid_attack_types:
            result.add_warning(
                f'Unusual attack type for data domain: {payload.attack_type}')

    def _validate_model_domain(self, payload: PayloadModel, result: ValidationResult
        ) ->None:
        """Validate model domain requirements."""
        valid_attack_types = ['theft', 'fingerprinting', 'evasion',
            'adversarial', 'model_inversion', 'watermarking']
        if payload.attack_type not in valid_attack_types:
            result.add_warning(
                f'Unusual attack type for model domain: {payload.attack_type}')

    def _validate_system_domain(self, payload: PayloadModel, result:
        ValidationResult) ->None:
        """Validate system domain requirements."""
        valid_attack_types = ['enumeration', 'privilege_escalation',
            'directory_traversal', 'information_disclosure',
            'configuration_bypass']
        if payload.attack_type not in valid_attack_types:
            result.add_warning(
                f'Unusual attack type for system domain: {payload.attack_type}'
                )

    def _validate_output_domain(self, payload: PayloadModel, result:
        ValidationResult) ->None:
        """Validate output domain requirements."""
        valid_attack_types = ['injection', 'content_steering',
            'format_string', 'template_injection', 'response_manipulation']
        if payload.attack_type not in valid_attack_types:
            result.add_warning(
                f'Unusual attack type for output domain: {payload.attack_type}'
                )

    def _validate_attack_vector_consistency(self, payload: PayloadModel, result:
        ValidationResult) ->None:
        """Validate attack vector consistency with other metadata."""
        type_to_vector = {'injection': AttackVector.INJECTION, 'jailbreak':
            AttackVector.EVASION, 'poisoning': AttackVector.POISONING,
            'enumeration': AttackVector.ENUMERATION, 'bypass': AttackVector
            .BYPASS}
    ...
    def _validate_content_patterns(self, payload: PayloadModel, result:
        ValidationResult) ->None:
        """Validate content against known patterns."""
        content = payload.content
        if payload.attack_vector == AttackVector.INJECTION:
            injection_patterns = self.content_patterns.get('injection', [])
            pattern_found = any(re.search(pattern, content, re.IGNORECASE) for
                pattern in injection_patterns)
            if not pattern_found:
                result.add_suggestion(
                    'Consider adding common injection patterns')

    def _validate_prompt_content(self, payload: PayloadModel, result:
        ValidationResult) ->None:
        """Validate prompt-specific content."""
        content = payload.content
        if not any(marker in content.lower() for marker in ['system:',
            'user:', 'assistant:', 'prompt:']):
            result.add_suggestion(
                'Consider adding role markers (system:, user:, etc.)')
        if len(content) < 10:
            result.add_warning('Prompt content seems very short')
        elif len(content) > 2000:
            result.add_warning(
                'Prompt content is quite long, may hit token limits')

    def _validate_data_content(self, payload: PayloadModel, result: ValidationResult
        ) ->None:
    ...
    def _has_good_documentation(self, payload: PayloadModel) ->bool:
        ...
        if len(desc) < 20:
            return False
        doc_keywords = ['purpose', 'usage', 'example', 'target', 'vector',
            'technique']
    ...
    def _get_quality_distribution(self, quality_scores: List[float]) ->Dict[
        ...
        return {'excellent': excellent, 'good': good, 'fair': fair, 'poor':
            poor}

    def _load_content_patterns(self) ->Dict[str, List[str]]:
        """Load content validation patterns."""
        return {'injection': ['select\\s+.*\\s+from', 'union\\s+select',
            '<script.*?>', 'javascript:', 'drop\\s+table', 'delete\\s+from',
            'insert\\s+into'], 'prompt': ['ignore\\s+previous',
            'system\\s*:', 'assistant\\s*:', 'user\\s*:', 'prompt\\s*:']}

    def _load_security_patterns(self) ->Dict[str, List[str]]:
        """Load security validation patterns."""
        return {'dangerous': ['rm\\s+-rf\\s+/', 'format\\s+c:',
            'exec\\s*\\(', 'eval\\s*\\(', 'system\\s*\\(',
            'shell_exec\\s*\\('], 'sensitive': ['password\\s*[:=]',
            'api[_-]?key\\s*[:=]', 'secret\\s*[:=]', 'token\\s*[:=]',
            'ssn\\s*[:=]', 'credit[_-]?card']}

    def _load_quality_weights(self) ->Dict[str, float]:
        """Load quality assessment weights."""
        return {'completeness': 0.4, 'metadata_quality': 0.3,
            'content_quality': 0.3}
