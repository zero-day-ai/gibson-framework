...
class TokenPrompter:
    ...
    def prompt_for_token(self, git_url: GitURL) -> Optional[str]:
        ...
        guidance = self._platform_guidance.get(
            platform, self._platform_guidance[GitPlatform.GENERIC]
        )
    ...
    def check_environment_token(self, git_url: GitURL) -> Optional[str]:
        ...
        env_patterns.extend(
            [f"GIT_TOKEN_{host_clean}", f"TOKEN_{host_clean}", "GIT_TOKEN", "SCM_TOKEN"]
        )
    ...
    def is_interactive_available(self) -> bool:
        ...
        """
        return self.interactive and sys.stdin.isatty() and sys.stdout.isatty()
    ...
    def get_auth_instructions(self, git_url: GitURL) -> str:
        ...
        if platform != GitPlatform.GENERIC:
            instructions.extend(
                [f"Create a {guidance['token_name']} at:", f"  {guidance['token_url']}", ""]
            )
