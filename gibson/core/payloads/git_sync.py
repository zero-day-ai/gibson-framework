...
class GitSync:
    ...
    def __init__(self, workspace_dir: Path, shallow: bool = True, config: Optional[GitSyncConfig] = None):
    ...
    async def clone_repository(
        ...
        try:
            ...
            logger.info(f"Successfully cloned {git_url.repo} in {duration:.2f}s using {auth_method.value}")
    ...
    async def update_repository(
        ...
        try:
            ...
            if updated:
                logger.info(f"Repository updated: {old_commit[:8]} -> {new_commit[:8]} in {duration:.2f}s")
            else:
        ...
        **operation_kwargs
        ...
        if token:
            try:
                ...
                if "authentication" in str(e).lower() or "permission" in str(e).lower():
                    raise AuthenticationError(f"Token authentication failed: {e}", AuthMethod.FAILED)
                else:
                    raise GitOperationError(f"Git operation failed: {e}")
        else:
