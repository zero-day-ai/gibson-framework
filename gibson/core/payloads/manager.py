...
class PayloadManager:
        ...
        try:
            ...
            if repo_path.exists() and not force_update:
                ...
                if update_result.success:
                    logger.info(f"Repository updated: {git_url.repo} (auth: {update_result.auth_method_used.value})")
                    
                    # Process payloads from repository
                    new_payloads = await self._process_repository_payloads(repo_path, target_domains)
                ...
                if clone_result.success:
                    logger.info(f"Repository cloned: {git_url.repo} (auth: {clone_result.auth_method_used.value})")
    ...
    async def import_payloads(
        ...
        try:
            ...
            if source_path.is_dir():
                files_to_import = list(source_path.glob("**/*.txt")) + \
            ...
            for file_path in payload_files:
                try:
                    ...
                    for payload in payloads_to_process:
                        ...
                        if target_domains and hasattr(payload, 'domain') and payload.domain not in target_domains:
                            continue
    ...
    async def cleanup_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            end_time = datetime.utcnow()
            results["total_cleanup_time_ms"] = int(
