...
class AIService:
    ...
    def __init__(self, config: Config):
        ...
        """
        self.config = config
        self.http_client = httpx.AsyncClient(timeout=config.api.timeout,
            verify=config.api.verify_ssl, headers={'User-Agent':
            'Gibson-Security-Framework'})
        self.rate_limiters = {provider: RateLimiter(config.api.rate_limit) for
            provider in AIProvider}
        self.provider_configs = self._load_provider_configs()
        self.stats = {'total_requests': 0, 'successful_requests': 0,
            'failed_requests': 0, 'total_tokens': 0, 'providers_used': {}}

    def _load_provider_configs(self) ->Dict[AIProvider, Dict[str, Any]]:
        """Load provider-specific configurations."""
        return {AIProvider.OPENAI: {'base_url': 'https://api.openai.com/v1',
            'models': ['gpt-4', 'gpt-4-turbo', 'gpt-3.5-turbo'],
            'default_model': 'gpt-4', 'max_tokens': 4096, 'headers': {
            'Authorization': 'Bearer {api_key}'}}, AIProvider.ANTHROPIC: {
            'base_url': 'https://api.anthropic.com/v1', 'models': [
            'claude-3-5-sonnet-20241022', 'claude-3-haiku-20240307'],
            'default_model': 'claude-3-5-sonnet-20241022', 'max_tokens': 
            4096, 'headers': {'x-api-key': '{api_key}', 'anthropic-version':
            '2023-06-01'}}, AIProvider.GOOGLE: {'base_url':
            'https://generativelanguage.googleapis.com/v1', 'models': [
            'gemini-1.5-pro', 'gemini-1.5-flash'], 'default_model':
            'gemini-1.5-pro', 'max_tokens': 4096, 'headers': {
            'Authorization': 'Bearer {api_key}'}}, AIProvider.AZURE: {
            'base_url':
            'https://{resource}.openai.azure.com/openai/deployments/{deployment}'
            , 'models': ['gpt-4', 'gpt-35-turbo'], 'default_model': 'gpt-4',
            'max_tokens': 4096, 'headers': {'api-key': '{api_key}'}},
            AIProvider.LOCAL: {'base_url': 'http://localhost:11434/v1',
            'models': ['llama2', 'mistral', 'codellama'], 'default_model':
            'llama2', 'max_tokens': 4096, 'headers': {}}}

    def _get_api_key(self, provider: AIProvider) ->Optional[str]:
        """Get API key for provider from environment or config."""
        import os
        key_names = {AIProvider.OPENAI: ['OPENAI_API_KEY',
            'GIBSON_OPENAI_KEY'], AIProvider.ANTHROPIC: [
            'ANTHROPIC_API_KEY', 'GIBSON_ANTHROPIC_KEY'], AIProvider.GOOGLE:
            ['GOOGLE_API_KEY', 'GIBSON_GOOGLE_KEY'], AIProvider.AZURE: [
            'AZURE_OPENAI_KEY', 'GIBSON_AZURE_KEY'], AIProvider.LOCAL: []}
    ...
    async def send_prompt(self, prompt: str, model: Optional[str]=None,
        provider: AIProvider=AIProvider.OPENAI, temperature: float=0.7,
        ...
        return await self.send_with_context(messages=messages, model=model,
            provider=provider, temperature=temperature, max_tokens=max_tokens)

    async def send_with_context(self, messages: List[Message], model:
        Optional[str]=None, provider: AIProvider=AIProvider.OPENAI,
        ...
        try:
            ...
            request_data = await self._build_request(provider, messages,
                model, temperature, max_tokens, provider_config)
            response = await self._send_request(provider, request_data,
                provider_config)
            ai_response = await self._parse_response(provider, response,
                model, start_time)
            ...
            return AIResponse(content='', provider=provider.value, model=
                model or 'unknown', response_time=time.time() - start_time,
                error=str(e))

    async def _build_request(self, provider: AIProvider, messages: List[
        Message], model: str, temperature: float, max_tokens: int,
        provider_config: Dict[str, Any]) ->Dict[str, Any]:
        """Build provider-specific request payload."""
        if provider == AIProvider.OPENAI:
            return {'model': model, 'messages': [{'role': msg.role,
                'content': msg.content} for msg in messages], 'temperature':
            ...
            for msg in messages:
                if msg.role == 'system':
                    ...
                    conversation_msgs.append({'role': msg.role, 'content':
                        msg.content})
            request = {'model': model, 'messages': conversation_msgs,
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1,
        min=4, max=10))
    async def _send_request(self, provider: AIProvider, request_data: Dict[
        str, Any], provider_config: Dict[str, Any]) ->httpx.Response:
    ...
    async def _parse_response(self, provider: AIProvider, response: httpx.
        Response, model: str, start_time: float) ->AIResponse:
        """Parse provider-specific response format."""
        response_data = response.model_dump_json()
        response_time = time.time() - start_time
        if (provider == AIProvider.OPENAI or provider == AIProvider.AZURE or
            ...
            tokens_used = None
        return AIResponse(content=content, provider=provider.value, model=
            model, tokens_used=tokens_used, response_time=response_time,
            metadata={'raw_response': response_data})

    def _update_stats(self, provider: AIProvider, response: Optional[
        AIResponse], success: bool) ->None:
