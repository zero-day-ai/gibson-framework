...
class TargetManager:
    ...
    async def create_target(self, 
        ...
        try:
            ...
            detected_provider = provider_hint or self.litellm_adapter.auto_detect_provider(
                url=base_url,
            ...
            logger.info(f"Created target '{name}' with provider {detected_provider.value}")
    ...
    async def update_target(self, 
        ...
        try:
            ...
            if 'base_url' in updates:
                target.provider = self.litellm_adapter.auto_detect_provider(
                    url=target.base_url,
        ...
        try:
            ...
            if with_credentials:
                for target in targets:
                    ...
                    target.metadata['credential_status'] = auth_status.get('validation_status', 'unknown')
    ...
    async def validate_target(self, 
                            ...
                            validate_credentials: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if test_connection:
                try:
                    ...
                    if not connectivity_result['success']:
                        results['connectivity_errors'] = [connectivity_result.get('error', 'Connection failed')]
                except Exception as e:
            ...
            if validate_credentials and target.requires_auth:
                try:
                    ...
                    if not cred_result.get('is_valid'):
                        results['credentials_errors'] = [cred_result.get('error_message', 'Credential validation failed')]
                except Exception as e:
            ...
            results['overall_valid'] = (
                ...
                (results['connectivity_valid'] is None or results['connectivity_valid']) and
                (results['credentials_valid'] is None or results['credentials_valid'])
            )
    ...
    async def set_target_credential(self, 
                                  identifier: Union[str, UUID],
                                  api_key: str,
                                  key_format: Union[str, ApiKeyFormat] = ApiKeyFormat.BEARER_TOKEN,
                ...
                if validate:
                    try:
                        ...
                        if validation_result.get('is_valid'):
                            ...
                            logger.warning(f"Credential validation failed for target '{target.name}': {validation_result.get('error_message')}")
                    except Exception as e:
                        logger.warning(f"Failed to validate credential for target '{target.name}': {e}")
    ...
    async def export_targets(self, 
                           file_path: Union[str, Path],
                           include_credentials: bool = False,
                           filter_kwargs: Optional[Dict] = None) -> int:
    ...
    async def import_targets(self, 
                           file_path: Union[str, Path],
                           update_existing: bool = False,
                           skip_credentials: bool = True) -> Dict[str, int]:
    ...
    async def _validate_target(self, target: TargetModel) -> None:
        ...
        if target.provider:
            provider_config = self.litellm_adapter.get_provider_config(
                ...
                url=target.base_url,
                model_name=self.litellm_adapter.get_litellm_model_name(
                    provider=target.provider,
                    url=target.base_url
                )
            )
