...
class Base:
    ...
    async def _discover_modules(self) -> None:
        ...
        logger.info(f"Discovered {len(self.available_modules)} modules across {len(AttackDomain)} domains")
    ...
    async def _execute_scan(self, target: str, modules: List[str], require_confirmation: bool) -> List[Finding]:
        ...
        tasks = [
            self._execute_module(target, module, semaphore)
            for module in modules
        ]
    ...
    async def _execute_module(self, target: str, module_name: str, semaphore: asyncio.Semaphore) -> Optional[Finding]:
...
def get_base_orchestrator(config: Optional[Config] = None, context: Optional[Context] = None) -> Base:
