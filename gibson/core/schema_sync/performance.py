    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_size_bytes": sum(
                len(pickle.dumps(e)) for e in self.memory_cache.values()
            ),
...
class ChangeDetectionOptimizer:
    ...
    def detect_changes_optimized(
        ...
        results = self.processor.process_parallel(
            list(all_fields),
            compare_field
        )
...
class GenerationOptimizer:
    ...
    async def generate_formats_async(
        self,
        model: type,
        formats: List[str]
    ) -> Dict[str, Any]:
        ...
        async def generate_format(format_name: str):
            ...
            if format_name == "json":
                from gibson.core.schema_sync.generators.json_generator import (
                    JSONSchemaGenerator
                )
                generator = JSONSchemaGenerator()
            elif format_name == "typescript":
                from gibson.core.schema_sync.generators.typescript import (
                    TypeScriptGenerator
                )
