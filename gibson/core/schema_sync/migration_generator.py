...
class AlembicMigrationGenerator:
    ...
    def generate_migration(
        self, changes: ChangeAnalysis, version: str, previous_revision: Optional[str] = None
    ) -> MigrationScript:
        """
        ...
        logger.info(
            f"Generated migration {revision_id} with {migration.total_statements} statements"
        )
        return migration
    ...
    def generate_rollback(self, changes: ChangeAnalysis) -> RollbackPlan:
        ...
        compatibility_value = (
            changes.compatibility.value
            if hasattr(changes.compatibility, "value")
            else changes.compatibility
        )
        data_backup_required = len(changes.changeset.removed_fields) > 0 or compatibility_value in [
            "major_breaking",
            "data_loss",
        ]
    ...
    def _generate_downgrade_sql(self, changes: ChangeAnalysis) -> List[str]:
        ...
        if changes.changeset.removed_fields:
            sql_statements.append("-- WARNING: Removed columns cannot be restored without backup")

        # Reverse type changes (simplified - would need original types)
        for field_name, modification in changes.changeset.modified_fields.items():
            if modification.change_type == FieldChangeType.TYPE_CHANGED:
                # This would need the original type stored
                sql_statements.append(f"-- TODO: Restore original type for {field_name}")
    ...
    def _generate_data_migrations(self, changes: ChangeAnalysis) -> List[DataMigration]:
        ...
        for field_name, modification in changes.changeset.modified_fields.items():
            if (
                modification.change_type == FieldChangeType.NULLABLE_CHANGED
                and modification.new_value is False
            ):
    ...
    def _generate_pre_checks(self, changes: ChangeAnalysis) -> List[str]:
        ...
        for field_name, modification in changes.changeset.modified_fields.items():
            if (
                ...
                checks.append(f"SELECT COUNT(*) FROM payloads WHERE {field_name} IS NULL;")
