...
class VersionRegistry:
    ...
    def register_version(
        self, version: str, schemas: Dict[str, Any], hash: str, model_name: str = "PayloadModel"
    ) -> None:
        """
    ...
    def record_migration(
        ...
        self._save_registry()
        logger.info(
            f"Recorded migration {migration_id} with status {status.value if hasattr(status, 'value') else status}"
        )
