...
class PreflightChecker:
    ...
    def check_backup_directory(self) -> PreflightCheck:
        ...
        backup_dir = Path(
            self.config.get("backup_directory", Path.home() / ".gibson" / "schema_backups")
        )
    ...
    def check_dependencies(self) -> PreflightCheck:
        ...
        required_packages = ["pydantic", "sqlalchemy", "aiosqlite"]
        ...
        check.details = {"installed": installed, "missing": missing}
    ...
    def check_permissions(self) -> PreflightCheck:
        ...
        migrations_dir = Path(
            self.config.get("migrations_directory", Path.cwd() / "gibson" / "migrations")
        )
    ...
    def generate_report(result: PreflightResult) -> str:
        ...
        for category, checks in sorted(by_category.items()):
            ...
            for check in checks:
                ...
                severity_marker = {"critical": "🔴", "warning": "🟡", "info": "🔵"}.get(
                    check.severity, ""
                )
