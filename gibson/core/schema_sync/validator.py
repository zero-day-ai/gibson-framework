...
class MigrationValidator:
    ...
    PERFORMANCE_PATTERNS = [
        (r"\bCREATE\s+INDEX\s+(?!CONCURRENTLY)", "Non-concurrent index creation can lock table"),
        (
    ...
    def validate(self, migration: MigrationScript, strict: bool = False) -> ValidationResult:
        """
    ...
    def _check_dangerous_patterns(self, migration: MigrationScript) -> List[ValidationIssue]:
    ...
    def _check_safety_patterns(self, migration: MigrationScript) -> List[ValidationIssue]:
        """Check for required safety patterns."""
        issues = []
        sql = f"{migration.upgrade_sql}\n{migration.downgrade_sql}"
    ...
    def _check_performance_patterns(self, migration: MigrationScript) -> List[ValidationIssue]:
        """Check for performance issues."""
        issues = []
        sql = f"{migration.upgrade_sql}\n{migration.downgrade_sql}"
    ...
    def _validate_sql_syntax(self, migration: MigrationScript) -> List[ValidationIssue]:
        """Validate SQL syntax (basic checks)."""
        issues = []

        # Check for basic syntax errors
        for sql_type, sql in [
            ("upgrade", migration.upgrade_sql),
    ...
    def _validate_rollback(self, migration: MigrationScript) -> List[ValidationIssue]:
    ...
    def _validate_data_integrity(self, migration: MigrationScript) -> List[ValidationIssue]:
    ...
    def _apply_custom_rules(self, migration: MigrationScript) -> List[ValidationIssue]:
        ...
        for pattern, message, severity in self.custom_rules:
            ...
            for match in matches:
                ...
                issues.append(
                    ValidationIssue(
                        severity=severity, category="custom", message=message, line_number=line_num
                    )
                )

        return issues

    def add_custom_rule(self, pattern: str, message: str, severity: str = "warning"):
        """
