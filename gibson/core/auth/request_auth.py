...
from gibson.models.auth import ApiKeyCredentialModel, ApiKeyFormat, AuthErrorType, ValidationStatus
...
class RequestAuthenticator:
    ...
    def authenticate_request(self, request: httpx.Request, target: TargetModel) -> httpx.Request:
    ...
    def _build_authentication_query_params(
        self, credential: ApiKeyCredentialModel
    ) -> Dict[str, str]:
    ...
    def handle_auth_response(self, response: httpx.Response) -> str:
        ...
        if status_code == 200:
            ...
            logger.warning(
                f"Authentication forbidden: {response.status_code} - {response.text[:100]}"
            )
            return AuthenticationStatus.INVALID_CREDENTIALS
    ...
    async def authenticated_request(
        self, client: httpx.AsyncClient, method: str, url: str, target: TargetModel, **kwargs
    ) -> httpx.Response:
