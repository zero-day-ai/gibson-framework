...
class AuthenticationConfig(BaseModel):
    ...
    credentials_dir: Optional[Path] = Field(
        default=None, description="Directory for credential storage"
    )
    encryption_enabled: bool = Field(default=True, description="Enable credential encryption")
    validation_enabled: bool = Field(default=True, description="Enable credential validation")
    validation_interval_hours: int = Field(
        default=24, ge=1, le=168, description="Hours between credential validations"  # 1 week max
    )
    cache_enabled: bool = Field(default=True, description="Enable credential caching")
    cache_ttl_minutes: int = Field(
        default=60, ge=5, le=1440, description="Cache TTL in minutes"  # 24 hours max
    )
    audit_enabled: bool = Field(default=True, description="Enable authentication audit logging")


class EnvironmentCredential(BaseModel):
    """Environment variable credential configuration."""

    target_name: str = Field(description="Target name for this credential")
    environment_variable: str = Field(description="Environment variable name")
    key_format: str = Field(default="bearer_token", description="API key format")
    validation_endpoint: Optional[str] = Field(default=None, description="Endpoint for validation")
    ...
    try:
        ...
        if not creds_dir.parent.exists():
            results["warnings"].append(
                f"Configuration directory does not exist: {creds_dir.parent}"
            )
        ...
        if creds_dir.exists():
            ...
            if dir_stat.st_mode & 0o077:  # Check if group/other have permissions
                results["warnings"].append(
                    f"Credentials directory has insecure permissions: {creds_dir}"
                )
