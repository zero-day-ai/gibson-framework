...
from gibson.models.auth import ApiKeyCredentialModel, AuthenticationValidationResult, ValidationStatus, AuthErrorType, RateLimitInfo, ApiKeyFormat
...
class AuthenticationService:
    ...
    def __init__(self, credential_manager: Optional[CredentialManager]=None,
        ...
        self.http_client = httpx.AsyncClient(timeout=timeout, verify=True,
            headers={'User-Agent': 'Gibson-Security-Framework/1.0'}, limits
    ...
    async def validate_credential(self, target: TargetModel, credential:
        Optional[ApiKeyCredentialModel]=None) ->AuthenticationValidationResult:
        ...
        try:
            if credential is None:
                try:
                    credential = self.credential_manager.retrieve_credential(
                        target.id)
                    if not credential:
                        return self._create_validation_result(False,
                            ValidationStatus.INVALID, AuthErrorType.
                            AUTHENTICATION_FAILED,
                            'No credential found for target',
                            response_time_ms=(time.time() - start_time) * 1000)
                except Exception as e:
                    return self._create_validation_result(False,
                        ValidationStatus.NETWORK_ERROR, AuthErrorType.
                        STORAGE_ERROR,
            ...
            if not credential.token:
                return self._create_validation_result(False,
                    ValidationStatus.INVALID, AuthErrorType.
                    INVALID_KEY_FORMAT, 'API key token is empty',
                    response_time_ms=(time.time() - start_time) * 1000)
            validation_url = self._get_validation_endpoint(target, credential)
            if not validation_url:
                return self._create_validation_result(False,
                    ValidationStatus.INVALID, AuthErrorType.
                    INVALID_ENDPOINT, 'No validation endpoint available',
                    response_time_ms=(time.time() - start_time) * 1000)
            result = await self._perform_validation_request(validation_url,
                credential, target)
            if result.is_valid:
                await self._update_credential_after_validation(target.id,
                    result)
            return result
        except Exception as e:
            logger.error(f'Validation failed for target {target.id}: {e}')
            return self._create_validation_result(False, ValidationStatus.
                UNKNOWN_ERROR, AuthErrorType.CONFIGURATION_ERROR,
                f'Validation error: {e}', response_time_ms=(time.time() -
                start_time) * 1000)

    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1,
        min=1, max=10))
    async def _perform_validation_request(self, url: str, credential:
        ApiKeyCredentialModel, target: TargetModel
        ...
        try:
            ...
            response_time_ms = (time.time() - start_time) * 1000
            return self._parse_validation_response(response, credential,
                response_time_ms, url)
        except httpx.TimeoutException as e:
            return self._create_validation_result(False, ValidationStatus.
                NETWORK_ERROR, AuthErrorType.NETWORK_TIMEOUT,
                f'Validation request timed out: {e}', response_time_ms=(
                time.time() - start_time) * 1000, validation_endpoint=url)
        except httpx.NetworkError as e:
            return self._create_validation_result(False, ValidationStatus.
                NETWORK_ERROR, AuthErrorType.NETWORK_TIMEOUT,
                f'Network error during validation: {e}', response_time_ms=(
                time.time() - start_time) * 1000, validation_endpoint=url)
        except Exception as e:
            return self._create_validation_result(False, ValidationStatus.
                UNKNOWN_ERROR, AuthErrorType.CONFIGURATION_ERROR,
                f'Unexpected validation error: {e}', response_time_ms=(time
                .time() - start_time) * 1000, validation_endpoint=url)

    def _build_auth_headers(self, credential: ApiKeyCredentialModel) ->Dict[
        str, str]:
        ...
        if credential.key_format == ApiKeyFormat.BEARER_TOKEN:
            ...
            headers[credential.header_name
                ] = credential.header_value or credential.token
    ...
    def _get_validation_endpoint(self, target: TargetModel, credential:
        ApiKeyCredentialModel) ->Optional[str]:
        ...
        if 'openai' in base_url.lower(
            ) or credential.key_format == ApiKeyFormat.OPENAI_FORMAT:
            ...
            return f'{base_url}/v1/models'
        common_paths = ['/health', '/status', '/api/health', '/api/status',
            '/v1/health', '/v1/status', '/']
        return f'{base_url}{common_paths[0]}'

    def _parse_validation_response(self, response: httpx.Response,
        credential: ApiKeyCredentialModel, response_time_ms: float,
        validation_endpoint: str) ->AuthenticationValidationResult:
        """Parse HTTP response to determine validation result."""
        rate_limit_info = self._extract_rate_limit_info(response)
        if response.status_code == 200:
            return self._create_validation_result(True, ValidationStatus.
                VALID, None, 'Credential validation successful',
                status_code=response.status_code, response_time_ms=
                response_time_ms, rate_limit_info=rate_limit_info,
                validation_endpoint=validation_endpoint)
        elif response.status_code == 401:
            error_message = self._extract_error_message(response)
            return self._create_validation_result(False, ValidationStatus.
                INVALID, AuthErrorType.AUTHENTICATION_FAILED, error_message or
                'Invalid API key', status_code=response.status_code,
                response_time_ms=response_time_ms, validation_endpoint=
                validation_endpoint)
        elif response.status_code == 403:
            error_message = self._extract_error_message(response)
            return self._create_validation_result(False, ValidationStatus.
                INSUFFICIENT_PERMISSIONS, AuthErrorType.
                INSUFFICIENT_PERMISSIONS, error_message or
                'Insufficient permissions', status_code=response.
                status_code, response_time_ms=response_time_ms,
                validation_endpoint=validation_endpoint)
        elif response.status_code == 429:
            error_message = self._extract_error_message(response)
            return self._create_validation_result(False, ValidationStatus.
                RATE_LIMITED, AuthErrorType.RATE_LIMIT_EXCEEDED, 
                error_message or 'Rate limit exceeded', status_code=
                response.status_code, response_time_ms=response_time_ms,
                rate_limit_info=rate_limit_info, validation_endpoint=
                validation_endpoint)
        elif response.status_code >= 500:
            error_message = self._extract_error_message(response)
            return self._create_validation_result(False, ValidationStatus.
                NETWORK_ERROR, AuthErrorType.SERVICE_UNAVAILABLE, 
                error_message or f'Server error: {response.status_code}',
                status_code=response.status_code, response_time_ms=
                response_time_ms, validation_endpoint=validation_endpoint)
        else:
            error_message = self._extract_error_message(response)
            return self._create_validation_result(False, ValidationStatus.
                UNKNOWN_ERROR, AuthErrorType.CONFIGURATION_ERROR, 
    ...
    def _extract_rate_limit_info(self, response: httpx.Response) ->Optional[
        RateLimitInfo]:
        ...
        try:
            ...
            if 'x-ratelimit-reset' in headers:
                reset_timestamp = datetime.fromtimestamp(int(headers[
                    'x-ratelimit-reset']))
            ...
            if any([requests_remaining, reset_timestamp, retry_after]):
                return RateLimitInfo(requests_remaining=requests_remaining,
                    reset_timestamp=reset_timestamp, retry_after=retry_after)
            ...
            if response.headers.get('content-type', '').startswith(
                ...
                for key in ['error', 'message', 'detail', 'error_description']:
                    if key in error_data:
                        if isinstance(error_data[key], dict
                            ) and 'message' in error_data[key]:
    ...
    def _create_validation_result(self, is_valid: bool, status:
        ValidationStatus, error_type: Optional[AuthErrorType]=None,
        error_message: Optional[str]=None, status_code: Optional[int]=None,
        response_time_ms: Optional[float]=None, rate_limit_info: Optional[
        RateLimitInfo]=None, validation_endpoint: Optional[str]=None
        ) ->AuthenticationValidationResult:
        """Create validation result object."""
        result = AuthenticationValidationResult(is_valid=is_valid,
            validation_status=status, status_code=status_code,
            response_time_ms=response_time_ms, error_type=error_type,
            error_message=error_message, rate_limit_info=rate_limit_info,
            validation_endpoint=validation_endpoint)
        if error_type == AuthErrorType.AUTHENTICATION_FAILED:
            ...
            result.add_recommendation(
                'Verify API key has required permissions/scopes')
            result.add_recommendation(
    ...
    async def _update_credential_after_validation(self, target_id: UUID,
        result: AuthenticationValidationResult) ->None:
        """Update credential metadata after successful validation."""
        try:
            self.credential_manager.update_credential_metadata(target_id,
                validation_status=result.validation_status, last_validated=
                result.validation_timestamp)
        except Exception as e:
            logger.warning(
                f'Failed to update credential metadata for {target_id}: {e}')
    ...
    async def test_authentication(self, target: TargetModel) ->Dict[str, Any]:
        ...
        """
        test_results = {'target_id': str(target.id), 'target_name': target.
            name, 'timestamp': datetime.utcnow().isoformat(),
            'overall_success': False, 'tests': []}
        try:
            ...
            validation_test = await self._test_credential_validation(target,
                credential)
            test_results['tests'].append(validation_test)
            if validation_test['success']:
                rate_limit_test = await self._test_rate_limits(target,
                    credential)
                test_results['tests'].append(rate_limit_test)
            test_results['overall_success'] = all(test['success'] for test in
                test_results['tests'])
        except Exception as e:
            test_results['tests'].append({'test_name':
                'authentication_test_error', 'success': False, 'error': str
                (e), 'timestamp': datetime.utcnow().isoformat()})
        return test_results

    async def _test_credential_retrieval(self, target: TargetModel) ->Dict[
        str, Any]:
        """Test credential retrieval for target."""
        test_result = {'test_name': 'credential_retrieval', 'success': 
            False, 'timestamp': datetime.utcnow().isoformat()}
        try:
    ...
    async def _test_credential_validation(self, target: TargetModel,
        credential: ApiKeyCredentialModel) ->Dict[str, Any]:
        """Test credential validation."""
        test_result = {'test_name': 'credential_validation', 'success': 
            False, 'timestamp': datetime.utcnow().isoformat()}
        try:
            validation_result = await self.validate_credential(target,
                credential)
            test_result['success'] = validation_result.is_valid
            test_result['validation_status'
            ...
            if not validation_result.is_valid:
                ...
                test_result['error_type'] = (validation_result.error_type.
                    value if validation_result.error_type else None)
    ...
    async def _test_rate_limits(self, target: TargetModel, credential:
        ApiKeyCredentialModel) ->Dict[str, Any]:
        """Test rate limit detection."""
        test_result = {'test_name': 'rate_limit_detection', 'success': True,
