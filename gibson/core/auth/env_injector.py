...
class EnvironmentCredentialInjector:
    ...
    def inject_credential(self, cred_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if existing:
                logger.debug(
                    f"Credential already exists for target {target_id}, skipping injection"
                )
                return True
            ...
            if self.validate_on_inject:
                ...
                if result.status != ValidationStatus.VALID:
                    logger.warning(
                        f"Injected credential for {target_id} failed validation: {result.error_message}"
                    )
                else:
    ...
    def generate_env_template(self, target_ids: List[str] = None) -> str:
        ...
        if target_ids:
            lines.extend(["# Specific target configurations:", ""])
    ...
    def export_env_file(
        self, file_path: Path, target_ids: List[str] = None, include_values: bool = False
    ) -> None:
...
def detect_ci_environment() -> Dict[str, Any]:
    ...
    in_container = (
        ...
        or "container" in os.environ.get("container", "")
    )
...
def auto_inject_from_environment() -> bool:
    ...
    try:
        # Perform injection
        validate_on_inject = os.environ.get("GIBSON_VALIDATE_ON_INJECT", "").lower() in [
            "true",
            "1",
        ]

        injector = EnvironmentCredentialInjector(
            auto_inject=True, validate_on_inject=validate_on_inject
        )
