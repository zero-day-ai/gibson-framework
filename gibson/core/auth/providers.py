...
class APIProvider(ABC):
    ...
    @abstractmethod
    def detect_auth_error(self, response: httpx.Response) ->Tuple[
        AuthErrorType, str]:
...
class OpenAIProvider(APIProvider):
    ...
    def __init__(self):
        ...
        config = ProviderConfig(name='openai', base_url=
            'https://api.openai.com/v1', validation_endpoint='/models',
            key_format=ApiKeyFormat.BEARER_TOKEN, rate_limit_headers={
            'limit': 'x-ratelimit-limit-requests', 'remaining':
            'x-ratelimit-remaining-requests', 'reset':
            'x-ratelimit-reset-requests'}, error_patterns={
    ...
    def format_auth_header(self, api_key: str) ->Dict[str, str]:
        ...
        return {'Authorization': f'Bearer {api_key}', 'OpenAI-Beta':
            'assistants=v2'}
    ...
    def parse_rate_limit_info(self, response: httpx.Response) ->RateLimitInfo:
        ...
        return RateLimitInfo(limit=int(headers.get(
            'x-ratelimit-limit-requests', 0)) or None, remaining=int(
            headers.get('x-ratelimit-remaining-requests', 0)) or None,
...
class AnthropicProvider(APIProvider):
    ...
    def __init__(self):
        ...
        config = ProviderConfig(name='anthropic', base_url=
            'https://api.anthropic.com', validation_endpoint='/v1/messages',
            key_format=ApiKeyFormat.CUSTOM_HEADER, header_name='x-api-key',
            rate_limit_headers={'limit':
            'anthropic-ratelimit-requests-limit', 'remaining':
            'anthropic-ratelimit-requests-remaining', 'reset':
            'anthropic-ratelimit-requests-reset'}, error_patterns={
            'invalid_api_key': AuthErrorType.INVALID_CREDENTIALS,
    ...
    def parse_rate_limit_info(self, response: httpx.Response) ->RateLimitInfo:
        ...
        headers = response.headers
        return RateLimitInfo(limit=int(headers.get(
            'anthropic-ratelimit-requests-limit', 0)) or None, remaining=
            int(headers.get('anthropic-ratelimit-requests-remaining', 0)) or
            None, reset_at=int(headers.get(
            'anthropic-ratelimit-requests-reset', 0)) or None, retry_after=
            int(headers.get('retry-after', 0)) or None)
...
class GoogleAIProvider(APIProvider):
    ...
    def __init__(self):
        ...
        config = ProviderConfig(name='google', base_url=
            'https://generativelanguage.googleapis.com',
            validation_endpoint='/v1beta/models', key_format=ApiKeyFormat.
            QUERY_PARAMETER, query_param='key', error_patterns={
...
class AzureOpenAIProvider(APIProvider):
    ...
    def __init__(self, resource_name: str=None, deployment_name: str=None):
        ...
        config = ProviderConfig(name='azure_openai', base_url=
            f'https://{self.resource_name}.openai.azure.com',
            validation_endpoint=
            f'/openai/deployments/{self.deployment_name}/completions',
            key_format=ApiKeyFormat.CUSTOM_HEADER, header_name='api-key',
            error_patterns={'invalid_api_key': AuthErrorType.
            INVALID_CREDENTIALS, 'rate_limit': AuthErrorType.RATE_LIMITED})
        super().__init__(config)
    ...
    def get_validation_endpoint(self) ->str:
        ...
        return (
            f'{self.config.base_url}{self.config.validation_endpoint}?api-version=2024-02-01'
            )

    def parse_rate_limit_info(self, response: httpx.Response) ->RateLimitInfo:
        """Parse Azure rate limit headers."""
        headers = response.headers
        return RateLimitInfo(remaining=int(headers.get(
            'x-ratelimit-remaining-requests', 0)) or None, retry_after=int(
            headers.get('retry-after', 0)) or None)
...
class AWSBedrockProvider(APIProvider):
    ...
    def __init__(self):
        ...
        config = ProviderConfig(name='aws_bedrock', base_url=
            'https://bedrock-runtime.{region}.amazonaws.com',
            validation_endpoint='/model/{model_id}/invoke', key_format=
            ApiKeyFormat.CUSTOM_HEADER, error_patterns={
            'UnrecognizedClientException': AuthErrorType.
            INVALID_CREDENTIALS, 'ThrottlingException': AuthErrorType.
            RATE_LIMITED})
        super().__init__(config)

    def format_auth_header(self, api_key: str) ->Dict[str, str]:
        """Format AWS Bedrock authentication headers."""
        return {'Authorization': f'AWS4-HMAC-SHA256 Credential={api_key}',
            'Content-Type': 'application/json'}
