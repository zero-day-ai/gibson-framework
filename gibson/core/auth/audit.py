...
class AuditLogger:
    ...
    def _generate_message(self, event_type: AuditEventType, success: bool) -> str:
        ...
        messages = {
            AuditEventType.CREDENTIAL_CREATED: "Credential created successfully" if success else "Failed to create credential",
            AuditEventType.CREDENTIAL_UPDATED: "Credential updated successfully" if success else "Failed to update credential",
            AuditEventType.CREDENTIAL_DELETED: "Credential deleted successfully" if success else "Failed to delete credential",
            AuditEventType.CREDENTIAL_ACCESSED: "Credential accessed" if success else "Credential access denied",
            AuditEventType.CREDENTIAL_VALIDATED: "Credential validation successful" if success else "Credential validation failed",
    ...
    async def _check_security_concerns(self, event: AuditEvent) -> None:
        ...
        recent_failures = [
            e for e in list(self.memory_events)[-100:]
