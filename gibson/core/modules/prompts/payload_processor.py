...
class PayloadProcessor:
    ...
    async def get_payloads(
        ...
        try:
            # Check cache first
            cache_key = self._generate_cache_key(techniques, max_payloads, aggressiveness, target_compatibility)
            ...
            # Filter payloads
            filtered_payloads = self._filter_payloads(payloads, techniques, aggressiveness, target_compatibility)
        ...
        try:
            ...
            if aggressiveness:
                severity_mapping = {
                    ...
                    "aggressive": [SeverityLevel.LOW, SeverityLevel.MEDIUM, SeverityLevel.HIGH, SeverityLevel.CRITICAL, SeverityLevel.INFO]
                }
                query_params["severity"] = severity_mapping.get(aggressiveness, severity_mapping["moderate"])
    ...
    def _filter_payloads(
        ...
        for payload in payloads:
            ...
            if not self._is_payload_compatible(payload, techniques, aggressiveness, target_compatibility):
                continue
    ...
    def _is_payload_compatible(
        ...
        if target_compatibility:
            ...
            if payload_targets and not any(target in payload_targets for target in target_compatibility):
                return False
    ...
    async def update_payload_effectiveness(
        ...
        try:
            ...
            stats.avg_confidence = (
                (stats.avg_confidence * (stats.total_attempts - 1) + confidence) / stats.total_attempts
            )
            stats.avg_response_time = (
                (stats.avg_response_time * (stats.total_attempts - 1) + response_time) / stats.total_attempts
            )
