...
class ResultAnalyzer:
    ...
    async def generate_findings(
        ...
        try:
            ...
            logger.info(f"Generated {len(findings)} findings from {len(attack_results)} attack results")
            return findings
    ...
    def _group_results_by_technique(self, results: List[AttackResult]) -> Dict[AttackTechnique, List[AttackResult]]:
    ...
    async def _analyze_technique_results(
        ...
        primary_finding = await self._generate_primary_finding(target, technique, primary_result, config)
        ...
        for result in successful_results[1:]:
            if result.confidence >= 0.8:  # Only very high confidence additional findings
                additional_finding = await self._generate_additional_finding(target, technique, result, config)
    ...
    async def _generate_additional_finding(
        ...
        base_severity = self._calculate_severity(technique, result.confidence, result.detection_details)
        ...
        evidence = {
            ...
            "leaked_content_sample": result.leaked_content[:100] + "..." if result.leaked_content and len(result.leaked_content) > 100 else result.leaked_content,
    ...
    def _generate_summary_finding(
        ...
        avg_confidence = sum(f.confidence for f in high_severity_findings) / len(high_severity_findings)
        ...
        evidence = {
            ...
            "avg_confidence": avg_confidence,
            "techniques_used": list(set(r.context.technique.value for r in attack_results if r.success))
        }
    ...
    def _generate_finding_description(self, technique: AttackTechnique, result: AttackResult) -> str:
        ...
        base_description = descriptions.get(technique, f"System prompt leakage detected via {method}")
    ...
    def _generate_remediation(self, technique: AttackTechnique, result: AttackResult) -> str:
        ...
        base_remediation = base_remediations.get(technique, "Implement general prompt protection measures.")
        ...
        if additional_recommendations:
            base_remediation += " Additional recommendations: " + " ".join(additional_recommendations)
