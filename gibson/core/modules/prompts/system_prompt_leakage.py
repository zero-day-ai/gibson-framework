...
class SystemPromptLeakageModule(BaseModule):
    ...
    async def setup(self) -> None:
        ...
        try:
            ...
            self.attack_engine = AttackEngine(
                module=self, client_factory=self.client_factory, config=self.module_config
            )

            # Initialize detection engine
            self.detection_engine = DetectionEngine(config=self.module_config)
            await self.detection_engine.initialize()

            # Initialize result analyzer
            self.result_analyzer = ResultAnalyzer(
                detection_engine=self.detection_engine, config=self.module_config
            )
    ...
    async def run(self, target: Target, config: Optional[Dict[str, Any]] = None) -> List[Finding]:
        ...
        try:
            ...
            if not await self.validate_target(target):
                raise ValueError(
                    f"Target validation failed: {target.url if hasattr(target, 'url') else target}"
                )
            ...
            logger.info(
                f"Starting system prompt leakage testing against {target} with {len(payloads)} payloads"
            )
            ...
            logger.info(
                f"System prompt leakage testing completed: {len(findings)} findings in {execution_time:.2f}s"
            )
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "config": {
                ...
                "enabled_techniques": [
                    technique
                    for technique, enabled in self.module_config.techniques.model_dump().items()
                    if enabled
                ],
            },
        }
    ...
    async def _load_payloads(self, config: Dict[str, Any]) -> List[Any]:
        ...
        try:
            ...
            enabled_techniques = [
                technique for technique, enabled in config.get("techniques", {}).items() if enabled
            ]
    ...
    async def _execute_attacks(
        self, target: Target, payloads: List[Any], config: Dict[str, Any]
    ) -> List[AttackResult]:
    ...
    async def _generate_findings(
        self, target: Target, attack_results: List[AttackResult], config: Dict[str, Any]
    ) -> List[Finding]:
    ...
    async def _update_statistics(
        self, attack_results: List[AttackResult], execution_time: float
    ) -> None:
