...
class ProviderPool:
    ...
    async def remove_endpoint(self, endpoint: ProviderEndpoint) -> None:
        ...
        async with self._lock:
            ...
            # Clean up sessions
            sessions_to_remove = [
                session_id for session_id, ep in self.session_map.items()
                if ep == endpoint
            ]
        ...
        async with self._lock:
            ...
            for endpoint in self.all_endpoints:
                ...
                if not endpoint.is_healthy and endpoint.last_error:
                    ...
                    retry_after = min(
                        60 * (2 ** min(endpoint.consecutive_errors, 5)),
                        300  # Max 5 minutes
                    )
    ...
    async def get_fallback_sequence(
        ...
        if self.config.strategy == FallbackStrategy.SEQUENTIAL:
            ...
            if required_capabilities:
                capable_providers = [
                    p for p, caps in self.provider_capabilities.items()
    ...
    async def complete(
        ...
        for attempt, endpoint in enumerate(fallback_sequence):
            ...
            try:
                ...
                if endpoint.consecutive_errors >= self.fallback_manager.config.circuit_breaker_threshold:
