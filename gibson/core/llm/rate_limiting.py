...
class RateStatus(GibsonBaseModel):
    ...
    concurrent_remaining: Optional[int] = Field(default=None, description="Concurrent slots remaining")
    reset_time: Optional[datetime] = Field(default=None, description="When limits reset")
    retry_after: Optional[float] = Field(default=None, description="Suggested retry delay in seconds")
...
class ProviderLimits(GibsonBaseModel):
    ...
    # Advanced limits
    requests_per_second: Optional[int] = Field(default=None, gt=0, description="Maximum requests per second")
    tokens_per_second: Optional[int] = Field(default=None, gt=0, description="Maximum tokens per second")
    
    # Burst handling
    burst_requests: Optional[int] = Field(default=None, gt=0, description="Burst capacity for requests")
    ...
    # Algorithm selection
    algorithm: AlgorithmType = Field(default=AlgorithmType.TOKEN_BUCKET, description="Rate limiting algorithm")
...
class QueuedRequest(TimestampedModel):
    ...
    # Timing
    queued_at: datetime = Field(default_factory=datetime.utcnow, description="When request was queued")
    timeout_at: Optional[datetime] = Field(default=None, description="When request times out")
    
    # Future for async notification
    future: Optional[asyncio.Future] = Field(default=None, exclude=True, description="Async notification")
        ...
        async with self._lock:
            ...
            if success:
                ...
                if self._consecutive_errors[provider] >= self.circuit_breaker_threshold:
                    self._circuit_breaker_until[provider] = (
    ...
    async def acquire(
        ...
        try:
            ...
            backpressure_action = await self._backpressure_manager.should_apply_backpressure(
                ...
                avg_latency=sum(self._request_queue._wait_times) / len(self._request_queue._wait_times)
                if self._request_queue._wait_times else 0.0
            )
            ...
            if await rpm_bucket.acquire(1) and await tpm_bucket.acquire(estimated_tokens):
                ...
                if module_name and not await self._check_module_limits(module_name, estimated_tokens):
...
def create_rate_limiter(
    ...
    **kwargs
    ...
    if distributed:
        ...
        return DistributedRateLimiter(redis_url=redis_url, provider_limits=provider_limits, **kwargs)
    else:
