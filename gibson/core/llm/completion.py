...
class PromptTemplate(GibsonBaseModel):
    ...
    def create_request(
        self, variables: Dict[str, Any], role: str = "user", **kwargs
    ) -> CompletionRequest:
...
class ResponseCache:
    ...
    def put(
        self, request: CompletionRequest, response: CompletionResponse, ttl: Optional[int] = None
    ) -> None:
        ...
        entry = CacheEntry(key=key, response=response, expires_at=expires_at)
...
class UsageTracker(TimestampedModel):
    ...
    def _calculate_cost(
        self, model: str, usage: TokenUsage, provider: Optional[str]
    ) -> Optional[Decimal]:
        ...
        ) * cost_model.completion_cost_per_1k
    ...
    def _estimate_generic_cost(self, model: str, usage: TokenUsage) -> Optional[Decimal]:
        ...
        for model_key, costs in generic_costs.items():
            if model_key in model.lower():
                ...
                completion_cost = (Decimal(usage.completion_tokens) / Decimal(1000)) * costs[
                    "completion"
                ]
                return prompt_cost + completion_cost
    ...
    def add_cost_model(self, cost_model: CostModel) -> None:
        ...
        provider_str = (
            cost_model.provider
            if isinstance(cost_model.provider, str)
            else cost_model.provider.value
        )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        avg_tokens_per_request = (
            self.total_tokens / self.successful_requests if self.successful_requests > 0 else 0
        )
...
class ModelSelector:
    ...
    def add_model(self, capabilities: ModelCapabilities) -> None:
        ...
        provider_str = (
            ...
            if isinstance(capabilities.provider, str)
            else capabilities.provider.value
        )
    ...
    def _meets_requirements(
        self, capabilities: ModelCapabilities, requirements: Dict[str, Any]
    ) -> bool:
    ...
    def _optimize_selection(
        self, models: List[Tuple[str, ModelCapabilities]], optimize_for: str
    ) -> str:
...
class CompletionService:
    ...
    def __init__(
        ...
        # Optional components
        self.cache = (
            ResponseCache(max_size=cache_size, default_ttl=cache_ttl) if enable_caching else None
        )
        ...
        logger.info(
            f"Initialized completion service (caching={enable_caching}, tracking={enable_usage_tracking})"
        )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    logger.warning(
                        f"Completion attempt {attempt + 1} failed, retrying in {delay}s: {e}"
                    )
...
async def quick_complete(
    messages: List[Dict[str, str]], model: str = "gpt-3.5-turbo", **kwargs
) -> CompletionResponse:
    """Quick completion for simple use cases."""
    # Convert dict messages to ChatMessage objects
    chat_messages = [ChatMessage(role=msg["role"], content=msg["content"]) for msg in messages]

    request = CompletionRequest(messages=chat_messages, model=model, **kwargs)
...
async def quick_stream(
    ...
    request = CompletionRequest(messages=chat_messages, model=model, stream=True, **kwargs)
