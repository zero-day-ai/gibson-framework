...
class LiteLLMAsyncClient:
    ...
    def _convert_request(self, request: CompletionRequest, config: ProviderConfig) -> Dict[str, Any]:
        """Convert Gibson request to LiteLLM format."""
        ...
        try:
            ...
            health_status = HealthStatus(
                ...
                consecutive_failures=0 if is_healthy else 
                    self._health_status.get(provider, HealthStatus(
                        provider=provider_config.provider,
                        is_healthy=False,
                        last_check=datetime.utcnow()
            ...
            health_status = HealthStatus(
                ...
                consecutive_failures=self._health_status.get(provider, HealthStatus(
                    provider=provider_config.provider,
                    is_healthy=False,
                    last_check=datetime.utcnow()
                )).consecutive_failures + 1,
        ...
        while not self._closed:
            try:
                ...
                health_tasks = [
                    ...
                    for provider_id in self._provider_configs.keys()
                ]
