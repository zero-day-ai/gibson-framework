...
from sqlalchemy import Column, String, Text, Integer, Boolean, DateTime, JSON, ForeignKey, Float, Index, Numeric
...
class UsageTrackingRecord(Base):
    ...
    request_metadata = Column(JSON, default=dict)  # renamed from metadata to avoid SQLAlchemy conflict
...
class UsageAggregationRecord(Base):
    ...
    __table_args__ = (
        Index('idx_aggregation_unique', 'period', 'period_start', 'provider', 'model', 
              'module_name', 'scan_id', 'user_id', unique=True),
...
class BudgetRecord(Base):
    ...
    budget_type = Column(String(20), nullable=False, index=True)  # daily, weekly, monthly, yearly, total
...
class CostBreakdown(GibsonBaseModel):
    ...
    # By scan
    by_scan: Dict[str, Decimal] = Field(
        default_factory=dict, description="Cost breakdown by scan"
    )
...
class CostCalculator:
    ...
    def get_model_pricing(self, provider: str, model: str) -> Optional[Dict[str, Decimal]]:
        ...
        if (self._last_update is None or 
            datetime.utcnow() - self._last_update > timedelta(hours=24)):
            self._update_pricing_cache()
    ...
    def calculate_cost(
        ...
        completion_cost = (Decimal(str(usage.completion_tokens)) * pricing["completion"]) / Decimal("1000")
...
class BudgetManager:
    ...
    async def _refresh_budget_cache(self) -> None:
        ...
        async with self.db_manager.session_factory() as session:
            result = await session.execute(
                select(BudgetRecord).where(BudgetRecord.enabled == True)
            )
    ...
    async def check_budget(
        ...
        if (self._last_cache_update is None or 
            datetime.utcnow() - self._last_cache_update > timedelta(minutes=5)):
            await self._refresh_budget_cache()
    ...
    async def _reset_budget_period(self, budget: BudgetRecord) -> None:
        ...
        async with self.db_manager.session_factory() as session:
            ...
            await session.execute(
                select(BudgetRecord).where(BudgetRecord.id == budget.id)
            )
...
class UsageDatabase:
    ...
    async def get_usage_summary(
        ...
        async with self.db_manager.session_factory() as session:
            ...
            fallback_rate = (sum(1 for r in records if r.fallback_used) / total_requests * 100) if total_requests > 0 else 0.0
            
            # Build breakdowns
            provider_breakdown = defaultdict(lambda: {"requests": 0, "cost": Decimal("0"), "tokens": 0})
            model_breakdown = defaultdict(lambda: {"requests": 0, "cost": Decimal("0"), "tokens": 0})
            module_breakdown = defaultdict(lambda: {"requests": 0, "cost": Decimal("0"), "tokens": 0})
    ...
    async def aggregate_usage(
        ...
        async with self.db_manager.session_factory() as session:
            for period_start, period_end in boundaries:
                ...
                for (provider, model, module_name, scan_id, user_id), group_records in groups.items():
                    await self._create_aggregation_record(
                        session, period, period_start, period_end,
                        provider, model, module_name, scan_id, user_id,
                        group_records
                    )
    ...
    async def _create_aggregation_record(
        ...
        fallback_rate = (sum(1 for r in records if r.fallback_used) / total_requests) if total_requests > 0 else 0.0
        avg_retry_count = sum(r.retry_count for r in records) / total_requests if total_requests > 0 else 0.0
...
class UsageReporter:
    ...
    def _export_csv(self, summary: UsageSummary, period_start: datetime, period_end: datetime) -> bytes:
...
class UsageTracker:
    ...
    async def track_usage(
        ...
        async with self._batch_lock:
            ...
            if (len(self._batch_buffer) >= self._batch_size or 
                datetime.utcnow() - self._last_flush > timedelta(seconds=self._batch_timeout)):
    ...
    async def set_budget_limit(
        ...
        return await self.budget_manager.create_budget(
            name, budget_type, limit_amount, provider, model, module_name, user_id, hard_limit=hard_limit
        )
    ...
    async def get_usage_trends(
        ...
        async with self.db_manager.session_factory() as session:
            query = select(UsageAggregationRecord).where(
                UsageAggregationRecord.period == period.value,
