...
class SystemDomain(BaseAttack):
            ...
            if risk_score > 0.3:
                ...
                return Finding(
                    ...
                    evidence={
                        ...
                        "sample_ports": system_info.open_ports[:5]
                        if system_info.open_ports
                        else [],
    ...
    async def _execute_dependency_analysis(
        ...
        try:
            ...
            if dependency_info and (
                dependency_info.outdated_packages or dependency_info.security_advisories
            ):
                ...
                return Finding(
                    ...
                    evidence={
                        ...
                        "critical_vulnerabilities": [
                            adv
                            for adv in dependency_info.security_advisories
            ...
            if successful_escalations:
                return Finding(
                    ...
                    evidence={
                        ...
                        "escalated_privileges": list(
                            set(
                                priv
                                for r in successful_escalations
                                for priv in r.escalated_privileges
                            )
                        ),
                        "test_results": [
                            {"method": r.method, "risk_level": r.risk_level, "evidence": r.evidence}
                    ...
                    tags=TaxonomyMapper().map_finding(
                        AttackDomain.SYSTEM, ModuleCategory.INJECTION
                    ),
                )
    ...
    async def _execute_generic_system_attack(
        ...
        if payloads:
            return Finding(
                ...
                tags=TaxonomyMapper().map_finding(AttackDomain.SYSTEM, ModuleCategory.UNSPECIFIED),
            )
        return None
    ...
    async def _perform_port_scan(self, target: str, system_info: SystemInfo) -> Dict[str, Any]:
        ...
        for port in self.common_ports[:10]:  # Limit ports for performance
            try:
                ...
                if result == 0:
                    open_ports.append(
                        {"port": port, "state": "open", "service": self._get_service_name(port)}
                    )
        ...
        try:
            ...
            for tech, patterns in self.tech_fingerprints.items():
                for pattern in patterns:
                    if re.search(pattern.lower(), headers_str) or re.search(
