...
class ModelDomain(BaseAttack):
            ...
            if fingerprint.confidence_score > 0.5:
                ...
                return Finding(
                    ...
                    tags=TaxonomyMapper().map_finding(
                        AttackDomain.MODEL, ModuleCategory.FINGERPRINTING
                    ),
                )
        ...
        try:
            ...
            if extracted_info:
                return Finding(
                    ...
                    tags=TaxonomyMapper().map_finding(AttackDomain.MODEL, ModuleCategory.THEFT),
                )
            ...
            if successful_examples:
                return Finding(
                    ...
                    evidence={
                        ...
                        "sample_adversarial": successful_examples[0].__dict__
                        if successful_examples
                        else None,
                        "perturbation_methods": list(
                            set(ex.perturbation_method for ex in successful_examples)
                        ),
                    },
                    remediation="Implement adversarial training and input validation",
                    tags=TaxonomyMapper().map_finding(AttackDomain.MODEL, ModuleCategory.EVASION),
                )
    ...
    async def _execute_generic_model_attack(
        ...
        if payloads:
            return Finding(
                ...
                tags=TaxonomyMapper().map_finding(AttackDomain.MODEL, ModuleCategory.UNSPECIFIED),
            )
        return None
    ...
    async def _analyze_response_timing(
        self, target: str, test_prompts: List[str]
    ) -> Dict[str, Any]:
    ...
    async def _analyze_error_patterns(self, target: str, test_prompts: List[str]) -> Dict[str, Any]:
        ...
        return {"confidence": 0.7 if detected_errors else 0.2, "errors": detected_errors}
    ...
    async def _analyze_behavioral_patterns(
        ...
        return {"confidence": 0.8 if detected_patterns else 0.3, "patterns": detected_patterns}

    async def _analyze_statistical_patterns(
        self, target: str, test_prompts: List[str]
    ) -> Dict[str, Any]:
    ...
    async def _perform_adversarial_probing(
        self, target: str, test_prompts: List[str]
    ) -> Dict[str, Any]:
        """Perform adversarial probing."""
        # Simplified adversarial probing
        return {"confidence": 0.4, "hints": ["adversarial_robustness_detected"]}
    ...
    async def _detect_watermarks(self, target: str, test_prompts: List[str]) -> Dict[str, Any]:
        ...
        return {"confidence": 0.3, "hints": ["potential_watermark_detected"]}
        ...
        for probe_prompt in self.architecture_probes[:3]:  # Limit probes
            try:
                response = await self.ai_service.send_prompt(
                    probe_prompt, provider=AIProvider.OPENAI, temperature=0.1
                )
    ...
    def _extract_context_info(self, content: str) -> str:
        ...
        context_patterns = [r"(\d+)\s*token", r"(\d+k)\s*context", r"(\d+)\s*words"]
    ...
    async def generate_adversarial_examples(
        self, input_text: str, target: str
